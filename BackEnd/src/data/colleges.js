// ─────────────────────────────────────────────────────────────────────────────
// COLLEGE ALIAS MAP
// Each key is the canonical normalized ID.
// Each value is an array of all known variations (all lowercase).
// ─────────────────────────────────────────────────────────────────────────────

export const COLLEGE_ALIAS_MAP = {

  // ── IITs ──────────────────────────────────────────────────────────────────

  "iit_bombay": [
    "iit bombay", "iitb", "iit mumbai",
    "indian institute of technology bombay",
    "indian institute of technology, bombay",
    "indian institute of technology (bombay)",
  ],

  "iit_delhi": [
    "iit delhi", "iitd", "iit d", "iit-d",
    "indian institute of technology delhi",
    "indian institute of technology, delhi",
    "indian institute of technology (delhi)",
  ],



  "iit_madras": [
    "iit madras", "iitm", "iit chennai",
    "indian institute of technology madras",
    "indian institute of technology, madras",
    "indian institute of technology (madras)",
  ],

  "iit_kharagpur": [
    "iit kharagpur", "iitkgp", "iit kgp",
    "indian institute of technology kharagpur",
    "indian institute of technology, kharagpur",
    "indian institute of technology (kharagpur)",
  ],

  "iit_kanpur": [
    "iit kanpur", "iitk",
    "indian institute of technology kanpur",
    "indian institute of technology, kanpur",
    "indian institute of technology (kanpur)",
  ],

  "iit_roorkee": [
    "iit roorkee", "iitr",
    "indian institute of technology roorkee",
    "indian institute of technology, roorkee",
    "indian institute of technology (roorkee)",
    "university of roorkee", // old name
  ],

  "iit_guwahati": [
    "iit guwahati", "iitg",
    "indian institute of technology guwahati",
    "indian institute of technology, guwahati",
    "indian institute of technology (guwahati)",
  ],

  "iit_hyderabad": [
    "iit hyderabad", "iith",
    "indian institute of technology hyderabad",
    "indian institute of technology, hyderabad",
    "indian institute of technology (hyderabad)",
  ],

  "iit_indore": [
    "iit indore", "iiti",
    "indian institute of technology indore",
    "indian institute of technology, indore",
  ],

  "iit_bhu": [
    "iit bhu", "iit varanasi", "iit (bhu)",
    "indian institute of technology bhu",
    "indian institute of technology (bhu) varanasi",
    "indian institute of technology banaras hindu university",
  ],

  "iit_patna": [
    "iit patna", "iitp",
    "indian institute of technology patna",
    "indian institute of technology, patna",
  ],

  "iit_gandhinagar": [
    "iit gandhinagar", "iitgn",
    "indian institute of technology gandhinagar",
    "indian institute of technology, gandhinagar",
  ],

  "iit_jodhpur": [
    "iit jodhpur", "iitj",
    "indian institute of technology jodhpur",
    "indian institute of technology, jodhpur",
  ],

  "iit_mandi": [
    "iit mandi",
    "indian institute of technology mandi",
    "indian institute of technology, mandi",
  ],

  "iit_bhubaneswar": [
    "iit bhubaneswar", "iitbbs",
    "indian institute of technology bhubaneswar",
    "indian institute of technology, bhubaneswar",
  ],

  "iit_tirupati": [
    "iit tirupati",
    "indian institute of technology tirupati",
    "indian institute of technology, tirupati",
  ],

  "iit_dhanbad": [
    "iit dhanbad", "iit (ism)", "iit ism",
    "ism dhanbad", "indian school of mines",
    "indian institute of technology (ism) dhanbad",
    "indian institute of technology dhanbad",
  ],

  "iit_palakkad": [
    "iit palakkad",
    "indian institute of technology palakkad",
    "indian institute of technology, palakkad",
  ],

  "iit_ropar": [
    "iit ropar",
    "indian institute of technology ropar",
    "indian institute of technology, ropar",
  ],

  "iit_jammu": [
    "iit jammu",
    "indian institute of technology jammu",
    "indian institute of technology, jammu",
  ],

  "iit_dharwad": [
    "iit dharwad",
    "indian institute of technology dharwad",
    "indian institute of technology, dharwad",
  ],

  "iit_bhilai": [
    "iit bhilai",
    "indian institute of technology bhilai",
    "indian institute of technology, bhilai",
  ],

  "iit_goa": [
    "iit goa",
    "indian institute of technology goa",
    "indian institute of technology, goa",
  ],

  // ── IIMs ──────────────────────────────────────────────────────────────────

  "iim_ahmedabad": [
    "iim ahmedabad", "iima", "iim-a",
    "indian institute of management ahmedabad",
    "indian institute of management, ahmedabad",
  ],

  "iim_bangalore": [
    "iim bangalore", "iim bengaluru", "iimb", "iim-b",
    "indian institute of management bangalore",
    "indian institute of management, bangalore",
    "indian institute of management bengaluru",
  ],

  "iim_calcutta": [
    "iim calcutta", "iim kolkata", "iimc", "iim-c",
    "indian institute of management calcutta",
    "indian institute of management, calcutta",
    "indian institute of management kolkata",
  ],

  "iim_lucknow": [
    "iim lucknow", "iiml", "iim-l",
    "indian institute of management lucknow",
    "indian institute of management, lucknow",
  ],

  "iim_kozhikode": [
    "iim kozhikode", "iimk", "iim-k", "iim calicut",
    "indian institute of management kozhikode",
    "indian institute of management, kozhikode",
  ],

  "iim_indore": [
    "iim indore", "iimi", "iim-i",
    "indian institute of management indore",
    "indian institute of management, indore",
  ],

  "iim_shillong": [
    "iim shillong",
    "indian institute of management shillong",
    "indian institute of management, shillong",
  ],

  "iim_rohtak": [
    "iim rohtak",
    "indian institute of management rohtak",
    "indian institute of management, rohtak",
  ],

  "iim_ranchi": [
    "iim ranchi",
    "indian institute of management ranchi",
    "indian institute of management, ranchi",
  ],

  "iim_raipur": [
    "iim raipur",
    "indian institute of management raipur",
    "indian institute of management, raipur",
  ],

  "iim_trichy": [
    "iim trichy", "iim tiruchirappalli",
    "indian institute of management tiruchirappalli",
    "indian institute of management trichy",
  ],

  "iim_udaipur": [
    "iim udaipur",
    "indian institute of management udaipur",
    "indian institute of management, udaipur",
  ],

  "iim_kashipur": [
    "iim kashipur",
    "indian institute of management kashipur",
    "indian institute of management, kashipur",
  ],

  "iim_nagpur": [
    "iim nagpur",
    "indian institute of management nagpur",
    "indian institute of management, nagpur",
  ],

  "iim_visakhapatnam": [
    "iim visakhapatnam", "iim vizag",
    "indian institute of management visakhapatnam",
    "indian institute of management, visakhapatnam",
  ],

  "iim_amritsar": [
    "iim amritsar",
    "indian institute of management amritsar",
    "indian institute of management, amritsar",
  ],

  "iim_bodhgaya": [
    "iim bodhgaya",
    "indian institute of management bodhgaya",
    "indian institute of management, bodhgaya",
  ],

  "iim_jammu": [
    "iim jammu",
    "indian institute of management jammu",
    "indian institute of management, jammu",
  ],

  "iim_sirmaur": [
    "iim sirmaur",
    "indian institute of management sirmaur",
    "indian institute of management, sirmaur",
  ],

  "iim_sambalpur": [
    "iim sambalpur",
    "indian institute of management sambalpur",
    "indian institute of management, sambalpur",
  ],

  // ── NITs ──────────────────────────────────────────────────────────────────

  "nit_trichy": [
    "nit trichy", "nit tiruchirappalli", "nitt",
    "national institute of technology trichy",
    "national institute of technology tiruchirappalli",
    "national institute of technology, tiruchirappalli",
    "regional engineering college trichy", // old name
  ],

  "nit_warangal": [
    "nit warangal", "nitw",
    "national institute of technology warangal",
    "national institute of technology, warangal",
    "regional engineering college warangal",
  ],

  "nit_surathkal": [
    "nit surathkal", "nit karnataka", "nitk",
    "national institute of technology surathkal",
    "national institute of technology karnataka",
    "national institute of technology, surathkal",
  ],

  "nit_calicut": [
    "nit calicut", "nit kozhikode", "nitc",
    "national institute of technology calicut",
    "national institute of technology, calicut",
    "regional engineering college calicut",
  ],

  "nit_rourkela": [
    "nit rourkela", "nitr",
    "national institute of technology rourkela",
    "national institute of technology, rourkela",
    "regional engineering college rourkela",
  ],

  "nit_allahabad": [
    "nit allahabad", "mnnit", "mnnit allahabad",
    "motilal nehru national institute of technology",
    "motilal nehru national institute of technology allahabad",
    "national institute of technology allahabad",
  ],

  "nit_nagpur": [
    "nit nagpur", "vnit", "vnit nagpur",
    "visvesvaraya national institute of technology",
    "visvesvaraya national institute of technology nagpur",
    "national institute of technology nagpur",
  ],

  "nit_jaipur": [
    "nit jaipur", "mnit", "mnit jaipur",
    "malaviya national institute of technology",
    "malaviya national institute of technology jaipur",
    "national institute of technology jaipur",
  ],

  "nit_kurukshetra": [
    "nit kurukshetra", "nitkkr",
    "national institute of technology kurukshetra",
    "national institute of technology, kurukshetra",
    "regional engineering college kurukshetra",
  ],

  "nit_durgapur": [
    "nit durgapur", "nitdgp",
    "national institute of technology durgapur",
    "national institute of technology, durgapur",
    "regional engineering college durgapur",
  ],

  "nit_silchar": [
    "nit silchar", "nits",
    "national institute of technology silchar",
    "national institute of technology, silchar",
  ],

  "nit_patna": [
    "nit patna", "nitp",
    "national institute of technology patna",
    "national institute of technology, patna",
  ],

  "nit_surat": [
    "nit surat", "svnit",
    "sardar vallabhbhai national institute of technology",
    "sardar vallabhbhai national institute of technology surat",
    "national institute of technology surat",
  ],

  "nit_hamirpur": [
    "nit hamirpur", "nith",
    "national institute of technology hamirpur",
    "national institute of technology, hamirpur",
  ],

  "nit_bhopal": [
    "nit bhopal", "manit",
    "maulana azad national institute of technology",
    "maulana azad national institute of technology bhopal",
    "national institute of technology bhopal",
  ],

  // ── BITS ──────────────────────────────────────────────────────────────────

  "bits_pilani": [
    "bits pilani", "bits",
    "birla institute of technology and science",
    "birla institute of technology and science pilani",
    "birla institute of technology & science",
    "birla institute of technology & science, pilani",
  ],

  "bits_goa": [
    "bits goa", "bits pilani goa campus",
    "birla institute of technology and science goa",
    "birla institute of technology & science goa",
  ],

  "bits_hyderabad": [
    "bits hyderabad", "bits pilani hyderabad campus",
    "birla institute of technology and science hyderabad",
    "birla institute of technology & science hyderabad",
  ],

  "bits_dubai": [
    "bits dubai", "bits pilani dubai campus",
    "birla institute of technology and science dubai",
  ],

  // ── Central Universities (all 54 UGC recognised) ──────────────────────────

  // ── Delhi (7 central universities) ───────────────────────────────────────

  "delhi_university": [
    "delhi university", "du", "university of delhi",
    "university of delhi, delhi",
  ],

  "jnu": [
    "jnu", "jawaharlal nehru university",
    "jawaharlal nehru university delhi",
    "jawaharlal nehru university, new delhi",
  ],

  "jamia_millia_islamia": [
    "jamia millia islamia", "jmi",
    "jamia millia islamia university",
    "jamia millia islamia, new delhi",
    "jamia university",
  ],

  "ignou": [
    "ignou",
    "indira gandhi national open university",
    "indira gandhi national open university, new delhi",
    "ignou new delhi",
  ],

  "iipa": [
    "iipa",
    "indian institute of public administration",
    "indian institute of public administration delhi",
  ],

  "south_asian_university": [
    "south asian university",
    "south asian university delhi",
    "sau delhi",
  ],

  "central_university_of_rajasthan": [
    "curaj", "central university of rajasthan",
    "central university rajasthan",
    "central university of rajasthan, ajmer",
  ],

  // ── Uttar Pradesh (6 central universities) ────────────────────────────────

  "bhu": [
    "bhu", "banaras hindu university",
    "banaras hindu university varanasi",
    "bhu varanasi",
  ],

  "amu": [
    "amu", "aligarh muslim university",
    "aligarh muslim university, aligarh",
    "aligarh university",
  ],

  "allahabad_university": [
    "allahabad university", "uoa",
    "university of allahabad",
    "university of allahabad, prayagraj",
    "allahabad university prayagraj",
    "allahabad central university",
  ],

  "bbau": [
    "bbau", "bbau lucknow",
    "babasaheb bhimrao ambedkar university",
    "babasaheb bhimrao ambedkar university lucknow",
    "dr. b.r. ambedkar university lucknow",
  ],

  "rmlnlu": [
    "rmlnlu", "ram manohar lohia national law university",
    "dr. ram manohar lohia national law university",
    "dr. ram manohar lohia national law university lucknow",
  ],

  "central_institute_higher_tibetan_studies": [
    "cihts", "central institute of higher tibetan studies",
    "central university of tibetan studies",
    "sarnath university",
  ],

  // ── Bihar (4 central universities) ────────────────────────────────────────

  "patna_university": [
    "patna university", "pu patna",
    "university of patna",
  ],

  "nalanda_university": [
    "nalanda university",
    "nalanda university rajgir",
  ],

  "mahatma_gandhi_central_university": [
    "mgcu", "mgcu bihar",
    "mahatma gandhi central university",
    "mahatma gandhi central university motihari",
    "mahatma gandhi central university, bihar",
  ],

  "central_university_of_south_bihar": [
    "cusb", "central university of south bihar",
    "central university south bihar",
    "central university of south bihar gaya",
  ],

  // ── Andhra Pradesh (3 central universities) ───────────────────────────────

  "eflu": [
    "eflu", "eflu hyderabad",
    "english and foreign languages university",
    "english and foreign languages university hyderabad",
    "central institute of english and foreign languages",
    "ciefl",
  ],

  "central_tribal_university_ap": [
    "ctua", "central tribal university of andhra pradesh",
    "central tribal university andhra pradesh",
    "central tribal university vizianagaram",
  ],

  "central_university_of_andhra_pradesh": [
    "cuap", "central university of andhra pradesh",
    "central university andhra pradesh",
    "central university of andhra pradesh anantapur",
  ],

  // ── Assam (2 central universities) ────────────────────────────────────────

  "tezpur_university": [
    "tezpur university",
    "tezpur university assam",
    "tezpur central university",
  ],

  "assam_university": [
    "assam university", "assam university silchar",
    "assam central university",
  ],

  // ── Jammu & Kashmir (2 central universities) ──────────────────────────────

  "central_university_of_kashmir": [
    "cuk", "central university of kashmir",
    "central university kashmir",
    "central university of kashmir srinagar",
  ],

  "central_university_of_jammu": [
    "cuj", "central university of jammu",
    "central university jammu",
    "central university of jammu, samba",
  ],

  // ── Manipur (3 central universities) ─────────────────────────────────────

  "manipur_university": [
    "manipur university",
    "manipur university imphal",
    "manipur central university",
  ],

  "central_agricultural_university": [
    "cau", "central agricultural university",
    "central agricultural university imphal",
  ],

  "national_sports_university": [
    "nsu manipur", "national sports university",
    "national sports university imphal",
  ],

  // ── Remaining States (one each) ───────────────────────────────────────────

  "hyderabad_university": [
    "university of hyderabad", "uoh", "hcu",
    "hyderabad central university",
    "hyderabad university",
    "central university of hyderabad",
  ],

  "central_university_of_gujarat": [
    "cug", "central university of gujarat",
    "central university gujarat",
    "central university of gujarat gandhinagar",
  ],

  "central_university_of_haryana": [
    "cuh", "central university of haryana",
    "central university haryana",
    "central university of haryana mahendergarh",
  ],

  "central_university_of_himachal_pradesh": [
    "cuhp", "central university of himachal pradesh",
    "central university himachal pradesh",
    "central university of himachal pradesh dharamshala",
    "central university of himachal pradesh, kangra",
  ],

  "central_university_of_jharkhand": [
    "cuj jharkhand", "central university of jharkhand",
    "central university jharkhand",
    "central university of jharkhand ranchi",
  ],

  "central_university_of_karnataka": [
    "cuk karnataka", "central university of karnataka",
    "central university karnataka",
    "central university of karnataka gulbarga",
    "central university of karnataka kalaburagi",
  ],

  "central_university_of_kerala": [
    "cuk kerala", "central university of kerala",
    "central university kerala",
    "central university of kerala kasaragod",
  ],

  "central_university_of_madhya_pradesh": [
    "cump", "central university of madhya pradesh",
    "central university madhya pradesh",
    "central university of madhya pradesh sagur",
  ],

  "maulana_azad_national_urdu_university": [
    "manuu", "maulana azad national urdu university",
    "maulana azad urdu university",
    "maulana azad national urdu university hyderabad",
  ],

  "central_university_of_orissa": [
    "cuo", "central university of orissa",
    "central university orissa",
    "central university of odisha",
    "central university of orissa koraput",
  ],

  "central_university_of_punjab": [
    "cup", "central university of punjab",
    "central university punjab",
    "central university of punjab bathinda",
  ],

  "central_university_of_rajasthan": [
    "curaj", "central university of rajasthan",
    "central university rajasthan",
    "central university of rajasthan ajmer",
  ],

  "central_university_of_tamilnadu": [
    "cutn", "central university of tamil nadu",
    "central university tamilnadu",
    "central university of tamil nadu thiruvarur",
  ],

  "hemvati_nandan_bahuguna_garhwal_university": [
    "hnbgu", "hemvati nandan bahuguna garhwal university",
    "hnb garhwal university",
    "garhwal university",
    "hemvati nandan bahuguna garhwal university srinagar",
  ],

  "mizoram_university": [
    "mizoram university",
    "mizoram university aizawl",
    "mizoram central university",
  ],

  "nagaland_university": [
    "nagaland university",
    "nagaland university kohima",
    "nagaland central university",
  ],

  "north_eastern_hill_university": [
    "nehu", "north eastern hill university",
    "north-eastern hill university",
    "north eastern hill university shillong",
  ],

  "rajiv_gandhi_university": [
    "rgu", "rajiv gandhi university",
    "rajiv gandhi university arunachal pradesh",
    "rajiv gandhi university itanagar",
  ],

  "sikkim_university": [
    "sikkim university",
    "sikkim university gangtok",
    "sikkim central university",
  ],

  "tripura_university": [
    "tripura university",
    "tripura university agartala",
    "tripura central university",
  ],

  "pondicherry_university": [
    "pondicherry university", "pondy university",
    "pondicherry university puducherry",
    "puducherry university",
  ],

  "dr_harisingh_gour_university": [
    "dr. harisingh gour university", "sagar university",
    "dr hari singh gour university",
    "dr. hari singh gour vishwavidyalaya",
    "sagar vishwavidyalaya",
  ],

  "mahatma_gandhi_antarrashtriya_hindi_vishwavidyalaya": [
    "mgahv", "mahatma gandhi antarrashtriya hindi vishwavidyalaya",
    "mahatma gandhi international hindi university",
    "hindi vishwavidyalaya wardha",
    "wardha university",
  ],

  "indira_gandhi_tribal_university": [
    "igtu", "indira gandhi tribal university",
    "indira gandhi national tribal university",
    "igtu amarkantak",
  ],

  "guru_ghasidas_vishwavidyalaya": [
    "ggv", "guru ghasidas vishwavidyalaya",
    "guru ghasidas university",
    "guru ghasidas vishwavidyalaya bilaspur",
    "guru ghasidas central university",
  ],

  // ── Other well-known non-central state/deemed universities ────────────────

  "jadavpur_university": [
    "jadavpur university", "ju",
    "jadavpur university kolkata",
  ],

  "anna_university": [
    "anna university", "anna university chennai",
    "anna university of technology",
  ],

  "pune_university": [
    "pune university", "sppu",
    "savitribai phule pune university",
    "university of pune",
  ],

  "mumbai_university": [
    "mumbai university", "university of mumbai",
    "bombay university", "university of bombay",
  ],

  "calcutta_university": [
    "calcutta university", "university of calcutta",
    "cu kolkata",
  ],

  "osmania_university": [
    "osmania university", "ou hyderabad",
    "osmania university hyderabad",
  ],

  "amity_university": [
    "amity university", "amity",
    "amity university noida",
    "amity university mumbai",
    "amity university jaipur",
  ],

  // ── IIITs ─────────────────────────────────────────────────────────────────

  "iiit_hyderabad": [
    "iiit hyderabad", "iiit-h", "iiith",
    "international institute of information technology hyderabad",
    "international institute of information technology, hyderabad",
  ],

  "iiit_allahabad": [
    "iiit allahabad", "iiita",
    "indian institute of information technology allahabad",
    "indian institute of information technology, allahabad",
  ],

  "iiit_bangalore": [
    "iiit bangalore", "iiitb",
    "international institute of information technology bangalore",
    "international institute of information technology, bangalore",
  ],

  "iiit_delhi": [
    "iiit delhi", "iiitd",
    "indraprastha institute of information technology delhi",
    "indraprastha institute of information technology, delhi",
  ],

  "iiit_gwalior": [
    "iiit gwalior", "iiitm gwalior",
    "atal bihari vajpayee indian institute of information technology and management gwalior",
    "abv-iiitm gwalior",
  ],

  // ── Popular Private Engineering Colleges ──────────────────────────────────

  "vit_vellore": [
    "vit", "vit vellore",
    "vellore institute of technology",
    "vellore institute of technology, vellore",
  ],

  "vit_chennai": [
    "vit chennai",
    "vellore institute of technology chennai",
  ],

  "vit_bhopal": [
    "vit bhopal",
    "vellore institute of technology bhopal",
  ],

  "vit_ap": [
    "vit ap", "vit amaravati",
    "vellore institute of technology andhra pradesh",
  ],

  "srm": [
    "srm", "srm university",
    "srm institute of science and technology",
    "srm university chennai",
    "srm engineering college",
  ],

  "manipal": [
    "manipal", "manipal university",
    "manipal institute of technology",
    "mit manipal",
    "mahe", "manipal academy of higher education",
  ],

  "thapar": [
    "thapar", "thapar university",
    "thapar institute of engineering and technology",
    "tiet patiala",
  ],

  "psg_coimbatore": [
    "psg tech", "psg college of technology",
    "psg college of technology coimbatore",
  ],

  "coimbatore_institute": [
    "cit coimbatore",
    "coimbatore institute of technology",
  ],

  "pes_bangalore": [
    "pes university", "pes",
    "pes institute of technology",
    "people's education society university",
  ],

  "rv_college": [
    "rv college", "rvce",
    "rv college of engineering",
    "r.v. college of engineering",
    "rv college of engineering bangalore",
  ],

  "bms_college": [
    "bms college", "bmsce",
    "bms college of engineering",
    "b.m.s. college of engineering",
  ],

  "ms_ramaiah": [
    "ms ramaiah", "msrit",
    "m.s. ramaiah institute of technology",
    "ramaiah institute of technology",
  ],

  "dtu": [
    "dtu", "delhi technological university",
    "delhi college of engineering", // old name
    "dce delhi",
  ],

  "nsit": [
    "nsit", "netaji subhas institute of technology",
    "netaji subhas university of technology", // renamed
    "nsut",
  ],

  "jiit": [
    "jiit", "jaypee institute of information technology",
    "jiit noida",
  ],

  "lnmiit": [
    "lnmiit", "lnm iit",
    "the lnm institute of information technology",
    "lnm institute of information technology jaipur",
  ],

  "mit_pune": [
    "mit pune",
    "maharashtra institute of technology pune",
    "mit college of engineering pune",
  ],

  "symbiosis": [
    "symbiosis", "sit pune",
    "symbiosis institute of technology",
    "symbiosis international university",
  ],

  // ── Popular Private MBA/Management Colleges ────────────────────────────────

  "xlri": [
    "xlri", "xlri jamshedpur",
    "xavier labour relations institute",
    "xavier school of management",
  ],

  "sp_jain": [
    "sp jain", "spjimr",
    "s.p. jain institute of management and research",
    "sp jain institute of management",
  ],

  "mdi_gurgaon": [
    "mdi", "mdi gurgaon",
    "management development institute",
    "management development institute gurgaon",
  ],

  "imt_ghaziabad": [
    "imt ghaziabad", "imt",
    "institute of management technology ghaziabad",
  ],

  "nmims": [
    "nmims", "nmims mumbai",
    "narsee monjee institute of management studies",
    "svkm's nmims",
  ],

  "sibm_pune": [
    "sibm pune", "sibm",
    "symbiosis institute of business management",
    "symbiosis institute of business management pune",
  ],

  "great_lakes": [
    "great lakes", "great lakes chennai",
    "great lakes institute of management",
  ],

  // ── Medical ───────────────────────────────────────────────────────────────

  "aiims_delhi": [
    "aiims", "aiims delhi", "aiims new delhi",
    "all india institute of medical sciences",
    "all india institute of medical sciences delhi",
    "all india institute of medical sciences, new delhi",
  ],

  "aiims_bhopal": [
    "aiims bhopal",
    "all india institute of medical sciences bhopal",
  ],

  "aiims_jodhpur": [
    "aiims jodhpur",
    "all india institute of medical sciences jodhpur",
  ],

  "aiims_rishikesh": [
    "aiims rishikesh",
    "all india institute of medical sciences rishikesh",
  ],

  "armed_forces_medical": [
    "afmc", "afmc pune",
    "armed forces medical college",
    "armed forces medical college pune",
  ],

  // ── Law ───────────────────────────────────────────────────────────────────

  "nlsiu": [
    "nlsiu", "nlsiu bangalore",
    "national law school of india university",
    "national law school bangalore",
  ],

  "nalsar": [
    "nalsar", "nalsar hyderabad",
    "nalsar university of law",
    "national academy of legal studies and research",
  ],

  "nlu_delhi": [
    "nlu delhi", "nlu",
    "national law university delhi",
    "national law university, delhi",
  ],

  // ── Design / Arts ─────────────────────────────────────────────────────────

  "nid": [
    "nid", "nid ahmedabad",
    "national institute of design",
    "national institute of design ahmedabad",
  ],

  "nift": [
    "nift", "nift delhi",
    "national institute of fashion technology",
    "national institute of fashion technology delhi",
  ],

  "iit_institute_of_design": [
    "idc", "idc iit bombay",
    "industrial design centre",
    "industrial design centre iit bombay",
  ],

};