...
const STUDENT_PROJECTION =
  'name email phone educations profileType skills experiences profileImage about jobRoles linkedin github portfolio lookingFor employmentType industry locations';
...
export const getStudentsByCollegeIdService = async (authUserId, profileType) => {
  ...
  const resolvedCollege = await resolveCollege(collegeName);

  // 2. Build query
  // const query = { college: { $regex: collegeName, $options: 'i' } };
  const query = {
    "educations.college_canonical_id": resolvedCollege.canonicalId,
  };
  ...
  const data = students.map(student => {
    const education = student.educations?.find(
      e => e.college_canonical_id === resolvedCollege.canonicalId
    );
