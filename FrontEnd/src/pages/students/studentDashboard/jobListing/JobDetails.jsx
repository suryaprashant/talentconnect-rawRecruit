...
const JobDetails = () => {
    ...
    useEffect(() => {
        const loadJobDetails = async () => {
            try {
                ...
                const data = Array.isArray(details.data) ? details.data[0] : details.data;
                
                setJobDetails(data);
                
                //view api
                if (data?._id) {
                    await viewed(data._id);
                }
    ...
    // Helper function to build company location string safely
    const getCompanyLocation = () => {
        const details = jobDetails?.companyPosted?.companyDetails;
        if (!details) return "Location not specified";
        const parts = [details.city, details.state, details.country].filter(Boolean);
        return parts.length > 0 ? parts.join(", ") : details.companyLocation || "Location not specified";
    };
                    ...
                    <div className="bg-white p-8 rounded-xl shadow-md space-y-8">
                        <header className="flex flex-col sm:flex-row items-start justify-between pb-4 border-b border-slate-200">
                            <div>
                                ...
                                <h3 className="text-xl font-bold text-slate-500 mb-3">
                                    {jobDetails.companyPosted?.companyDetails?.companyName || "Company Name"}
                                </h3>

                                {/* 2. Job Roles (Array) with small font */}
                                <div className="flex flex-wrap gap-2 mb-4">
                                    {jobDetails.jobRoles && jobDetails.jobRoles.length > 0 ? (
                                        jobDetails.jobRoles.map((role, index) => (
                                            <span 
                                                key={index} 
                                                className="bg-slate-100 border border-slate-200 text-slate-900 px-3 py-1 rounded-md text-sm font-bold"
                                            >
                                                {role}
                                            </span>
                                        ))
                                        ...
                                        <span className="text-xl font-bold text-slate-900">
                                            {jobDetails.jobTitle || "Job Opportunity"}
                                        </span>
                                    )}
                                </div>
                                ...
                                <div className="flex flex-wrap gap-x-6 gap-y-2">
                                    ...
                                    {jobDetails.yearsOfExperience && (
                                        <InfoPill 
                                            ...
                                            text={`${jobDetails.yearsOfExperience} years experience`} 
                                        />
                                    )}
                                </div>
                            </div>
                                ...
                                <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                                    ...
                                    <div className="text-xl font-bold text-slate-900">
                                        {jobDetails?.packageDetails?.totalCTC
                                            ? `${jobDetails.packageDetails.currency || 'INR'} ${jobDetails.packageDetails.totalCTC.toLocaleString()}`
                                            : 'Not Specified'}
                                    </div>
                                </div>
                                <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                                    ...
                                    <div className="text-xl font-bold text-slate-900">
                                        {jobDetails?.packageDetails?.fixedPay
                                            ? `${jobDetails.packageDetails.currency || 'INR'} ${jobDetails.packageDetails.fixedPay.toLocaleString()}`
                                            : 'N/A'}
                                    </div>
                                </div>
                                <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                                    ...
                                    <div className="text-xl font-bold text-slate-900">
                                        {jobDetails?.packageDetails?.joiningBonus
                                            ? `${jobDetails.packageDetails.currency || 'INR'} ${jobDetails.packageDetails.joiningBonus.toLocaleString()}`
                                            : 'N/A'}
                                    </div>
                                </div>
                            </div>
                        ...
                        <Section title="Education">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <p className="text-sm font-medium text-slate-500">Minimum Education / Degree</p>
                                    <p className="text-slate-700 capitalize font-semibold">
                                        {/* Using 'degree' array from JSON */}
                                        {jobDetails.degree?.length > 0 ? jobDetails.degree.join(", ") : (jobDetails.minEducation || "Not Specified")}
                                    </p>
                                </div>
                                <div>
                                    <p className="text-sm font-medium text-slate-500">Preferred Field of Study</p>
                                    <p className="text-slate-700 capitalize font-semibold">
                                        {/* studentStreams is an array in JSON */}
                                        {Array.isArray(jobDetails.studentStreams) ? jobDetails.studentStreams.join(", ") : jobDetails.studentStreams || "Any"}
                                    </p>
                                </div>
                            </div>
                        </Section>
                        ...
                        <Section title="Job Overview">
                            <div className="flex flex-wrap gap-y-4 gap-x-8">
                                <SnapshotListItem 
                                    ...
                                    label="Industry" 
                                    value={jobDetails.companyPosted?.companyDetails?.industryType || "Not Specified"} 
                                />
                                <SnapshotListItem 
                                    icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M2 5a2 2 0 012-2h12a2 2 0 012 2v2a2 2 0 01-2 2H4a2 2 0 01-2-2V5zm14 1a1 1 0 11-2 0 1 1 0 012 0zM2 13a2 2 0 012-2h12a2 2 0 012 2v2a2 2 0 01-2 2H4a2 2 0 01-2-2v-2zm14 1a1 1 0 11-2 0 1 1 0 012 0z" clipRule="evenodd" /></svg>} 
                                    label="Work Mode" 
                                    value={Array.isArray(jobDetails.workMode) ? jobDetails.workMode.join(", ") : jobDetails.workMode} 
                                />
                                
                                {/* REMOVED "Job Role" from here as requested */}
                                ...
                                <SnapshotListItem 
                                    ...
                                    label="Openings / Min Students" 
                                    value={jobDetails.numberOfOpenings || jobDetails.minimumStudents || "Not Specified"} 
                                />
                                <SnapshotListItem 
                                    ...
                                    label="Department" 
                                    value={jobDetails.department || "Not Specified"} 
                                />
                                <SnapshotListItem 
                                    icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M17.707 9.293a1 1 0 010 1.414l-7 7a1 1 0 01-1.414 0l-7-7A.997.997 0 012 10V5a3 3 0 013-3h5a.997.997 0 01.707.293l7 7zM5 6a1 1 0 100-2 1 1 0 000 2z" clipRule="evenodd" /></svg>} 
                                    label="Employment Type" 
                                    value={Array.isArray(jobDetails.employmentType) ? jobDetails.employmentType.join(", ") : jobDetails.employmentType} 
                                />
                                <SnapshotListItem 
                                    icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M5 2a2 2 0 00-2 2v11a3 3 0 003 3h8a3 3 0 003-3V4a2 2 0 00-2-2H5zm0 2h10v9a1 1 0 01-1 1H6a1 1 0 01-1-1V4z" clipRule="evenodd" /></svg>} 
                                    label="Certificate Required" 
                                    value={jobDetails.certifications?.length > 0 ? jobDetails.certifications.join(", ") : "No certification required"} 
                                />
                                <SnapshotListItem 
                                    icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM4.332 8.027a6.012 6.012 0 011.912-2.706C6.512 5.72 7.97 5 10 5c2.03 0 3.488.72 4.756 1.321l.003.001c.624.305 1.135.688 1.536 1.106l-1.091.819C14.72 7.82 14.166 7.5 13 7.5c-1.276 0-2.221.48-2.638.895l-.068.071c-.693.732-.888 1.816-.949 2.887l-2.024.091a4.502 4.502 0 01-1.008-2.324zM10 15a4.5 4.5 0 100-9 4.5 4.5 0 000 9z" clipRule="evenodd" /></svg>} 
                                    label="Work Authorization" 
                                    value={jobDetails.workAuthorization || 'Not Specified'} 
                                />
                            </div>
                        </Section>
                        ...
                        <Section title="About The Company">
                            ...
                            <p className="text-sm text-slate-500">
                                <span className="font-semibold text-slate-600">Location:</span> {getCompanyLocation()}
                            </p>
                        </Section>
                    </div>
                </main>
