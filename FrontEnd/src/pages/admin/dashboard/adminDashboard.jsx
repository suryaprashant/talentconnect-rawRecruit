...
const RelevancyWeightsModal = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    setFetching(true);
    axios
      .get(`${import.meta.env.VITE_Backend_URL}${ENDPOINTS[mode]}`, {
        headers: {
  ...
  const handleSlider = (key, val) => {
    setWeights((p) => ({ ...p, [mode]: { ...p[mode], [key]: Number(val) } }));
    setStatus(null);
  };
  const handleInput = (key, val) => {
    const n = val === "" ? 0 : Math.min(100, Math.max(0, Number(val) || 0));
    setWeights((p) => ({ ...p, [mode]: { ...p[mode], [key]: n } }));
    setStatus(null);
  };
  const handleReset = () => {
    setWeights((p) => ({ ...p, [mode]: { ...DEFAULTS[mode] } }));
    setStatus(null);
  };
  ...
  const availabilityKeys = WEIGHT_KEYS.filter(
    (k) => WEIGHT_META[k].group === "availability",
  );
  const academicsKeys = WEIGHT_KEYS.filter(
    (k) => WEIGHT_META[k].group === "academics",
  );
  const academicsSum = academicsKeys.reduce(
    (s, k) => s + (Number(w[k]) || 0),
    0,
  );
    ...
    >
      <div
        style={{
          ...
          animation: "slideUp .24s cubic-bezier(0.34,1.4,0.64,1)",
          overflow: "hidden",
        }}
      >
        {/* Header */}
        ...
        >
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
              ...
              <div>
                ...
                >
                  Configure match-score factors per candidate type
                </p>
              </div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
              <div
                style={{
                  ...
                  border: `1.5px solid ${isValid ? "rgba(167,243,208,0.7)" : "rgba(252,165,165,0.6)"}`,
                  borderRadius: "999px",
                  padding: "4px 12px",
                  fontSize: "13px",
                  fontWeight: 700,
                  fontFamily: "'DM Sans', sans-serif",
                  color: isValid ? "#d1fae5" : "#fee2e2",
                }}
              >
                {total}/100
              </div>
          ...
          >
            <TabPill
              ...
              icon={GraduationCap}
              label="Student"
              accentColor={modeAccent.student}
            />
            <TabPill
              ...
              icon={Briefcase}
              label="Professional"
              accentColor={modeAccent.professional}
            />
          </div>
          ...
          <div>
            ...
            >
              ...
              >
                Academics subtotal:{" "}
                <strong style={{ color: "#ede9fe" }}>{academicsSum}</strong>
              </span>
            </div>
          </div>
        </div>
          ...
          {fetching ? (
            ...
            <>
              ...
              {coreKeys.map((key) => (
                <WeightRow
                  key={key}
                  label={WEIGHT_META[key].label}
                  color={WEIGHT_META[key].color}
                  value={w[key]}
                  onChange={(v) => handleSlider(key, v)}
                  onInputChange={(v) => handleInput(key, v)}
                />
              ))}
              ...
              <SectionLabel
                label={`Academics  ·  subtotal ${academicsSum}`}
                style={{ marginTop: "20px" }}
              />
              ...
              {mode === "professional" && (
                ...
                >
                  💡 For professional candidates, CGPA and Batch Year are
                  typically set to <strong>0</strong> since work experience
                  carries more weight.
                </p>
              )}
            </>
        ...
        >
          {status === "success" && (
            ...
            >
              ...
              >
                {mode === "student" ? "Student" : "Professional"} weights
          ...
          {!isValid && !status && (
            ...
            >
              ...
              >
                Total must equal 100. Currently: <strong>{total}</strong>
                {total < 100
                  ? ` (${remaining} short)`
                  : ` (${total - 100} over)`}
              </span>
          ...
          <div style={{ display: "flex", gap: "10px" }}>
            ...
            <button
              ...
              onMouseEnter={(e) => {
                if (!saving && isValid) e.currentTarget.style.opacity = "0.88";
              }}
              onMouseLeave={(e) => {
            ...
            >
              {saving ? (
                ...
                <>
                  <BarChart3 size={14} />
                  Save {mode === "student" ? "Student" : "Professional"} Weights
                </>
              )}
            </button>
          </div>
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_Backend_URL}/api/company-jobs/addCompany-carrer`,
        {
...
const AdminDashboard = () => {
  ...
  const stats = [
    ...
    },
    {
      title: "Applications",
      value:
        dashboardData.totalJobApplicationSubmission ||
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="bg-white shadow rounded-lg p-6">
            ...
            <p className="text-gray-500 mb-4">
              Manage users, companies, and colleges
            </p>
            <button className="w-full bg-blue-600 text-white py-2 px-4 rounded-md hover:bg-[#143694]">
              Manage Users
            </button>
          </div>
          <div className="bg-white shadow rounded-lg p-6">
          ...
          <div className="bg-white shadow rounded-lg p-6">
            ...
            <button className="w-full bg-[#143694] text-white py-2 px-4 my-4 rounded-md hover:bg-purple-700">
              Open Settings
            </button>
          </div>
          <div
          ...
          >
            ...
            <p className="text-gray-500 mb-4" style={{ fontSize: "14px" }}>
              Set the match-score threshold to control which jobs candidates
              see.
            </p>
            <button
              className="w-full bg-orange-500 text-white py-2 px-4 rounded-md hover:bg-orange-600 font-medium text-sm"
              style={{
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
              ...
              <div>
                ...
                >
                  Fine-tune how each factor — skills, experience, academics,
                  location and more — contributes to candidate match scores.
                  Separate profiles for Students &amp; Professionals.
                </p>
              </div>
            </div>
            <div
            ...
            >
              ...
              <button
                className="bg-[#143694] text-white py-2 px-5 rounded-md hover:bg-purple-700 font-medium text-sm"
