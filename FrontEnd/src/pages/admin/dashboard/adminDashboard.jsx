...
const DEFAULTS = {
  student: {
    skills: 38, jobRoles: 7, experience: 4,
    noticePeriod: 2, noticePeriodDays: 2,
    cgpa: 6, batchYear: 5, location: 4,
    degree: 13, stream: 16, salary: 3,
  },
  professional: {
    skills: 40, jobRoles: 13, experience: 9,
    noticePeriod: 4, noticePeriodDays: 3,
    cgpa: 0, batchYear: 0, location: 4,
    degree: 9, stream: 13, salary: 5,
  },
};

const ENDPOINTS = {
  student:      '/api/admin/dashboard/relevancy-weights',
  professional: '/api/admin/dashboard/relevancy-weights-professional',
};

// Note: fix the typo in your router if you want to unify these
const PATCH_ENDPOINTS = {
  student:      '/api/admin/dashboard/relevancy-weights',
  professional: '/api/admin/dashboard/relevancy-weights-professioanl',
};
...
const SectionLabel = ({ label, style = {} }) => (
  <div style={{
    ...
    fontFamily: "'DM Sans', sans-serif", marginBottom: '10px', ...style,
  }}>
    {label}
  </div>
);
...
const WeightRow = ({ label, color, value, onChange, onInputChange }) => (
  <div className="rw-row" style={{
    display: 'grid', gridTemplateColumns: '130px 1fr 54px',
    alignItems: 'center', gap: '14px',
    padding: '9px 10px', borderRadius: '10px', marginBottom: '4px',
    transition: 'background .15s',
  }}>
    <span style={{ fontSize: '13.5px', fontWeight: 600, color: '#334155', fontFamily: "'DM Sans', sans-serif", whiteSpace: 'nowrap' }}>
      {label}
    </span>
    <input
      ...
      className="rw-slider"
      style={{ background: `linear-gradient(to right, ${color} ${value}%, #e2e8f0 ${value}%)`, color }}
    />
    <input
      ...
      style={{
        ...
        border: `1.5px solid ${color}44`, borderRadius: '7px',
        fontSize: '13px', fontWeight: 700,
        fontFamily: "'DM Sans', sans-serif", color,
        textAlign: 'center', background: `${color}08`, outline: 'none',
      }}
...
const TabPill = ({ active, onClick, icon: Icon, label, accentColor }) => (
  <button
    onClick={onClick}
    style={{
...
const RelevancyWeightsModal = ({ isOpen, onClose }) => {
  ...
  const [fetched, setFetched]   = useState({ student: false, professional: false });
  ...
  useEffect(() => {
    if (!isOpen) return;
    if (fetched[mode]) return;
    setStatus(null);
    setFetching(true);
    axios.get(
      `${import.meta.env.VITE_Backend_URL}${ENDPOINTS[mode]}`,
      {
        headers: {
      ...
      .then(res => {
        if (res.data?.success && res.data?.data) {
          ...
          const merged = {};
          for (const k of WEIGHT_KEYS) merged[k] = d[k] ?? DEFAULTS[mode][k];
          setWeights(prev => ({ ...prev, [mode]: merged }));
  ...
  useEffect(() => {
    if (!isOpen) {
      setFetched({ student: false, professional: false });
      setStatus(null);
    }
  }, [isOpen]);

  const w         = weights[mode];
  const total     = WEIGHT_KEYS.reduce((s, k) => s + (Number(w[k]) || 0), 0);
  const remaining = 100 - total;
  const isValid   = total === 100;

  const handleSlider  = (key, val) => { setWeights(p => ({ ...p, [mode]: { ...p[mode], [key]: Number(val) } })); setStatus(null); };
  const handleInput   = (key, val) => { const n = val === '' ? 0 : Math.min(100, Math.max(0, Number(val) || 0)); setWeights(p => ({ ...p, [mode]: { ...p[mode], [key]: n } })); setStatus(null); };
  const handleReset   = () => { setWeights(p => ({ ...p, [mode]: { ...DEFAULTS[mode] } })); setStatus(null); };
  ...
  const handleSave = async () => {
    ...
    try {
      await axios.patch(
        `${import.meta.env.VITE_Backend_URL}${PATCH_ENDPOINTS[mode]}`,
        weights[mode],
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('adminToken')}`,
  ...
  const academicsSum     = academicsKeys.reduce((s, k) => s + (Number(w[k]) || 0), 0);

  const headerBg   = 'linear-gradient(135deg, #7c3aed 0%, #6d28d9 100%)';
  const modeAccent = {
    student:      'rgba(99,102,241,0.85)',
    professional: 'rgba(14,165,233,0.85)',
  };
  ...
  return (
    <div ref={overlayRef} onClick={handleOverlay} style={{ position: 'fixed', inset: 0, background: 'rgba(107,114,128,0.5)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, animation: 'fadeIn .18s ease', padding: '16px' }}>
      <div style={{ background: '#fff', borderRadius: '16px', width: '100%', maxWidth: '600px', maxHeight: '92vh', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,0.18)', animation: 'slideUp .24s cubic-bezier(0.34,1.4,0.64,1)', overflow: 'hidden' }}>

        {/* Header */}
        <div style={{ background: headerBg, padding: '20px 24px 16px', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '14px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <div>
                ...
                <p style={{ margin: '3px 0 0', color: '#ede9fe', fontSize: '12.5px', fontFamily: 'sans-serif' }}>Configure match-score factors per candidate type</p>
              </div>
            </div>
          ...
          {/* Tab switcher */}
          <div style={{ display: 'flex', gap: '8px', background: 'rgba(0,0,0,0.15)', borderRadius: '999px', padding: '4px', width: 'fit-content', marginBottom: '14px' }}>
            <TabPill active={mode === 'student'}       onClick={() => { setMode('student');       setStatus(null); }} icon={GraduationCap} label="Student"      accentColor={modeAccent.student} />
            <TabPill active={mode === 'professional'}  onClick={() => { setMode('professional');  setStatus(null); }} icon={Briefcase}     label="Professional" accentColor={modeAccent.professional} />
          </div>
        ...
        <div style={{ overflowY: 'auto', flex: 1, padding: '20px 26px' }}>
          {fetching ? (
            <div style={{ textAlign: 'center', padding: '48px 0', color: '#94a3b8', fontFamily: 'sans-serif' }}>
              ...
              Loading {mode} weights…
            </div>
          ) : (
            <>
              <SectionLabel label="Core Factors" />
              {coreKeys.map(key => (
                <WeightRow key={key} label={WEIGHT_META[key].label} color={WEIGHT_META[key].color} value={w[key]} onChange={v => handleSlider(key, v)} onInputChange={v => handleInput(key, v)} />
              ))}
              ...
              {mode === 'professional' && (
                <p style={{ margin: '12px 10px 0', padding: '10px 14px', background: '#f0f9ff', border: '1px solid #bae6fd', borderRadius: '10px', fontSize: '12.5px', color: '#0369a1', fontFamily: 'sans-serif', lineHeight: 1.5 }}>
                  💡 For professional candidates, CGPA and Batch Year are typically set to <strong>0</strong> since work experience carries more weight.
                </p>
              )}
            </>
          )}
        ...
        <div style={{ padding: '16px 26px 20px', borderTop: '1px solid #f1f5f9', flexShrink: 0, background: '#fff' }}>
          {status === 'success' && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '10px', padding: '10px 14px', marginBottom: '14px' }}>
              ...
              <span style={{ color: '#15803d', fontSize: '13px', fontFamily: 'sans-serif' }}>{mode === 'student' ? 'Student' : 'Professional'} weights updated successfully!</span>
            </div>
          )}
          {status === 'error' && (
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            ...
            <button onClick={handleSave} disabled={!isValid || saving}
              ...
              {saving ? (
                ...
                <><BarChart3 size={14} />Save {mode === 'student' ? 'Student' : 'Professional'} Weights</>
              )}
            </button>
          </div>
        </div>
...
const AdminDashboard = () => {
  ...
  const [dashboardData, setDashboardData] = useState({
    totalUsers: 0, totalCompanies: 0, totalColleges: 0,
    totalApplications: 0, recentActivity: [],
  });
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-6">
            ...
            <div className="flex items-center space-x-4">
              <button className="p-2 text-gray-400 hover:text-gray-600"><Settings className="h-5 w-5" /></button>
              <button onClick={handleLogout} className="flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
              ...
              <div className="space-y-3">
                {dashboardData.recentActivity.map((activity, index) => (
                  <div key={index} className="flex items-center space-x-3">
                    <div className="flex-shrink-0"><TrendingUp className="h-5 w-5 text-gray-400" /></div>
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          >
            ...
            <p className="text-gray-500 mb-4" style={{ fontSize: '14px' }}>Set the match-score threshold to control which jobs candidates see.</p>
            <button className="w-full bg-orange-500 text-white py-2 px-4 rounded-md hover:bg-orange-600 font-medium text-sm" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
              <SlidersHorizontal size={14} />Update Threshold
            </button>
          </div>
        </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
              ...
              <div>
                ...
                <p className="text-gray-500" style={{ fontSize: '13.5px', maxWidth: '420px', lineHeight: 1.5 }}>
                  Fine-tune how each factor — skills, experience, academics, location and more — contributes to candidate match scores. Separate profiles for Students &amp; Professionals.
                </p>
              </div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '12px' }}>
              <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                {[['Student', 'bg-indigo-100 text-indigo-700'], ['Professional', 'bg-sky-100 text-sky-700'], ['Academics', 'bg-green-100 text-green-700'], ['Location', 'bg-teal-100 text-teal-700'], ['Salary', 'bg-amber-100 text-amber-700']].map(([f, cls]) => (
              ...
              <button className="bg-[#143694] text-white py-2 px-5 rounded-md hover:bg-purple-700 font-medium text-sm" style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
                <BarChart3 size={14} />Configure Weights
              </button>
            </div>
          </div>
        </div>
