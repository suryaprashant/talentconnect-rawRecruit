...
//       // Using the original employer endpoint
...
export default function PoolCampusHiringForm() {
  // ... (all your const options remain the same) ...
  const locations = ['Online', 'Bangalore', 'Mumbai', 'Delhi', 'Hyderabad', 'Chennai', 'Pune', 'Kolkata', 'Ahmedabad', 'Jaipur', 'Other'];
  const collegeStreamMapping = {
    'Engineering': ['B.Tech', 'M.Tech', 'Computer Science', 'Electronics', 'Mechanical', 'Civil', 'Electrical', 'Information Technology', 'Biotechnology', 'Chemical', 'Aerospace', 'Automobile'],
    'Management': ['MBA', 'BBA'],
  ...
  const initialState = {
    ...
    description: '',
    packageDetails: { currency: 'INR', totalCTC: '', fixedPay: '', joiningBonus: '' },
    workLocation: [], // This was 'workLocations' in your V1, check your backend
    jobRoles: [],
    workMode: [],
    employmentType: [],
    skills: [],
  ...
  const [indianCities, setIndianCities] = useState([]); // Will store {name, stateCode} objects
  const [workLocationSearch, setWorkLocationSearch] = useState('');
  ...
  // --- ⭐️ FIX 1: Store full city objects ---
  useEffect(() => {
    const citiesOfIndia = City.getCitiesOfCountry('IN')
      .sort((a, b) => a.name.localeCompare(b.name));
    setIndianCities(citiesOfIndia); // Store the full objects [{name, stateCode}, ...]
  }, []);
  ...
  // ... (handleChange, handleMultiSelect, removeSelectedItem, toggleDropdown, etc. are all OK) ...
  const handleChange = (e) => {
    const { name, value } = e.target;
    if (name === "description") {
      if (value.length > 500) {
  ...
  // --- ⭐️ FIX 2: Update search function to work with objects ---
  const getFilteredCities = (cities, searchTerm) => {
    if (!searchTerm.trim()) {
      return cities; // Returns array of objects
    }
    
    const searchLower = searchTerm.toLowerCase();
    const citiesWithPriority = cities.map(city => { // 'city' is an object
      const cityLower = city.name.toLowerCase(); // Search by name
      let priority = 0;
      ...
      return { city, priority }; // 'city' is the full object
    });
    
    return citiesWithPriority
      .filter(item => item.priority > 0)
      .sort((a, b) => b.priority - a.priority || a.city.name.localeCompare(b.city.name)) // Sort by name
      .map(item => item.city); // Return the filtered full objects
  };
  ...
  const handleSubmit = async (e) => {
    // ... (Your handleSubmit logic is fine, no changes needed) ...
    e.preventDefault();
    setError(null);
    if (formData.description.length > 500) {
      setDescriptionError("Job description cannot exceed 500 characters.");
    ...
    try {
      ...
      const submissionData = {
        ...
        workLocation: formData.workLocation, // Make sure backend expects 'workLocation' not 'location'
        jobRoles: formData.jobRoles,
        workMode: formData.workMode,
        employmentType: formData.employmentType,
        skills: formData.skills,
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 font-sans">
      ...
      <div className="mb-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* --- ⭐️ FIX 3: Update Venue Dropdown JSX --- */}
              ...
              <div className="absolute z-20 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg">
                ...
                <div className="max-h-60 overflow-auto">
                  {filteredVenueCities.length > 0 ? (
                    filteredVenueCities.map(city => ( // city is an object
                      <div
                        key={`${city.name}-${city.stateCode}`} // <-- UNIQUE KEY
                        onClick={() => {
                          setFormData(prev => ({ ...prev, venue: city.name })); // <-- Set string name
                          setDropdownOpen(prev => ({ ...prev, venue: false }));
                        }}
                        className={`px-4 py-2 hover:bg-gray-100 cursor-pointer ${formData.venue === city.name ? "bg-gray-100 font-medium" : ""}`} // <-- Compare name
                      >
                        {city.name} {/* <-- Render name */}
                        {formData.venue === city.name && <span className="float-right text-gray-500">✓</span>}
                      </div>
                    ))
                  ) : (
          ...
          {/* ... (Rest of the form - Eligibility, Description, Skills, Benefits, Tags, Amenities, Package, etc.) ... */}
          {/* All these sections are OK */}
          ...
          {/* --- ⭐️ FIX 4: Update Work Location Dropdown JSX --- */}
              ...
              <div className="absolute z-20 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg">
                ...
                <div className="max-h-60 overflow-auto">
                  {filteredWorkCities.length > 0 ? (
                    filteredWorkCities.map(city => ( // city is an object
                      <div
                        key={`${city.name}-${city.stateCode}`} // <-- UNIQUE KEY
                        onClick={() => handleMultiSelect('workLocation', city.name)} // <-- Pass string name
                        className={`px-4 py-2 hover:bg-gray-100 cursor-pointer ${formData.workLocation.includes(city.name) ? "bg-gray-100 font-medium" : ""}`} // <-- Compare name
                      >
                        {city.name} {/* <-- Render name */}
          ...
          {/* ... (Job Roles, Work Mode, Employment Type, etc. are all OK) ... */}
          <div ref={jobRolesRef} className="relative">
            <label className="block font-medium mb-2">Job Role <span className="text-red-500">*</span></label>
            <div className="flex flex-wrap gap-2 mb-2">
              {formData.jobRoles.map(role => (
