...
export default function PoolCampusHiringForm() {
  ...
  const collegeTypes = [ 'Engineering', 'Medical', 'Management', 'Arts & Science', 'Law', 'Pharmacy', 'Architecture', 'Polytechnic', 'ITI', 'Other' ];
  ...
  const initialState = {
    ...
    workLocations: '',
    jobRoles: [], // Changed to multi-select
    workMode: [],
    employmentType: [],
    skills: [],
    benefits: [],
    amenities: [], // Added new field
    placementStartDate: '',
    placementEndDate: '',
    numberOfRounds: '',
    selectionProcess: '',
  ...
  const [dropdownOpen, setDropdownOpen] = useState({
    ...
    skills: false,
    benefits: false,
    jobRoles: false, // Added for multi-select
    amenities: false, // Added for new field
  });

  const studentStreamsRef = useRef(null);
  const skillsRef = useRef(null);
  const benefitsRef = useRef(null);
  const jobRolesRef = useRef(null); // Added for multi-select
  const amenitiesRef = useRef(null); // Added for new field

  useEffect(() => {
    const handleClickOutside = (event) => {
      if (studentStreamsRef.current && !studentStreamsRef.current.contains(event.target)) {
        setDropdownOpen(prev => ({ ...prev, studentStreams: false }));
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const submissionData = {
        ...
        location: formData.workLocations ? [formData.workLocations] : [],
        jobRoles: formData.jobRoles,
        workMode: formData.workMode,
        employmentType: formData.employmentType,
        skills: formData.skills,
        benefits: formData.benefits,
        amenitiesRequired: formData.amenities, // Mapped to schema
    ...
    <div className="max-w-4xl mx-auto p-4 font-sans">
      ...
      <div className="mb-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label className="block mb-1 font-medium">Work Location <span className="text-red-500">*</span></label>
            <div className="relative">
              <select name="workLocations" value={formData.workLocations} onChange={handleChange} className="w-full p-2 border rounded appearance-none pr-8 bg-white" required>
                <option value="" disabled>Select work location</option>
                {workLocations.map((location) => (<option key={location} value={location}>{location}</option>))}
              </select>
              <div className="absolute inset-y-0 right-0 flex items-center px-2 pointer-events-none"><ChevronDown className="w-4 h-4 text-gray-400" /></div>
            </div>
          </div>
          
          {/* UPDATED: Job Role Multi-Select */}
