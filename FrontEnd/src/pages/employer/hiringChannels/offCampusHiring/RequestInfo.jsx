...
const [metaData, setMetaData] = useState([]); // Shared skills from DB
...
useEffect(() => {
  const fetchSkills = async () => {
    try {
      ...
      console.error("Error loading global skills", err);
    }
  };
  fetchSkills();
}, []);
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#667eea]/5 via-[#f093fb]/5 to-[#764ba2]/5 py-4">
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        <div className="bg-white/90 backdrop-blur-sm border border-gray-100 rounded-xl shadow p-6">
          <div className="text-center mb-6">
            <h2 className="text-2xl font-bold bg-gradient-to-r from-[#667eea] to-[#764ba2] bg-clip-text text-transparent">Register for Off-Campus Hiring</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* SECTION 2: Student & Eligibility Details */}
            <div className="pt-4 border-t border-gray-100">
              <div className="flex items-center mb-4">
                <div className="p-1.5 bg-gradient-to-br from-[#667eea]/20 to-[#764ba2]/20 rounded-lg mr-3">
                  <Users className="h-4 w-4 text-[#667eea]" />
                </div>
                <h3 className="text-lg font-semibold text-gray-800">Student & Eligibility Details</h3>
              </div>
                      ...
                      <div className="absolute z-20 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-hidden">
                        ...
                        <div className="overflow-y-auto max-h-48">
                          {availableStreams.length > 0 ? (
                            ...
                            <div className="p-4 text-center text-gray-400 text-xs italic">
                              No predefined streams for custom degree. Add manually above.
                            </div>
                          )}
                        </div>
                      </div>
...
<div ref={skillsRef} className="relative pt-2">
  ...
  `}>
    ...
    <input
      ...
      placeholder={formData.skills.length === 0 ? "Search skills (e.g. React, Docker)..." : "Add more..."}
  ...
  {isSkillDropdownOpen && (
    <div className="absolute z-50 w-full mt-2 bg-white border border-gray-100 rounded-xl shadow-2xl overflow-hidden animate-in slide-in-from-top-2 duration-200">
      <div className="max-h-60 overflow-y-auto">
        ...
        {/* "Add New" button if skill doesn't exist yet */}
        {customSkillSearch && !filteredSkillOptions.some(s => s.toLowerCase() === customSkillSearch.toLowerCase()) && (
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span>Add "<span className="underline italic">{customSkillSearch}</span>" to shared list</span>
            </div>
          </button>
        )}
        ...
        {customSkillSearch === "" && filteredSkillOptions.length === 0 && (
          <div className="px-5 py-8 text-center text-gray-400 text-xs italic">
            Start typing to search existing skills...
          </div>
        )}
      </div>
    </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {['Hybrid', 'On-site', 'Remote'].map(mode => (
                          <button 
                            ...
                            type="button" 
                            onClick={() => handleMultiSelect('workMode', mode)} 
                            className={`px-3 py-1.5 text-sm border rounded-lg transition-colors ${formData.workMode.includes(mode) ? 'bg-gradient-to-r from-[#667eea] to-[#764ba2] text-white border-transparent' : 'bg-white text-gray-700 border-gray-200 hover:border-gray-300'}`}
                          >
                            {mode}
                          </button>
                        ))}
