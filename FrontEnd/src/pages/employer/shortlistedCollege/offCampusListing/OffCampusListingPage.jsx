...
export default function OffCampusJobManagement() {
  ...
  const fetchJobs = async () => {
    ...
    setError(null);
    try {
      // Consistency: Fetching "Off-campus" with "Shortlisted" status
  ...
  // Enhanced Filter logic matching the "FileSearch" requirement
  const filteredJobs = jobs?.filter(job => {
    ...
    // Check work locations/modes
    const locationMatch = (job.location && Array.isArray(job.location))
      ? job.location.some(loc => loc?.toLowerCase().includes(searchLower))
      : job.workMode?.toLowerCase().includes(searchLower);

    const matchesSearch = jobRolesMatch || locationMatch || job._id?.toLowerCase().includes(searchLower);
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#667eea]/10 via-[#f093fb]/5 to-[#764ba2]/10">
      <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="flex flex-col md:flex-row items-center gap-4">
            <div className="relative flex-grow">
              ...
              <input
                type="text"
                className="w-full pl-10 pr-4 py-2.5 bg-gradient-to-r from-gray-50 to-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#667eea]/50 focus:border-transparent focus:outline-none transition-all duration-200"
                placeholder="Search by job role, location or ID"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
              />
            </div>
        ...
        <div className="bg-white/90 backdrop-blur-sm border border-gray-100 rounded-2xl shadow-lg overflow-hidden">
            ...
            {loading ? (
              ...
              currentJobs?.map(job => (
                <div key={job._id} className="p-4 hover:bg-gray-50/50 transition-all duration-200">
                  <div className="grid grid-cols-12 gap-4 items-center">
                    {/* Job Title - Clickable for Preview */}
                    <div className="col-span-4">
                      <button 
                    ...
                    <div className="col-span-2">
                      <div className="flex items-center justify-center gap-2">
                        <button 
                          onClick={() => handleView(job._id)}
                          className="p-2 bg-gradient-to-r from-gray-100 to-white border border-gray-200 text-gray-600 rounded-lg hover:text-[#667eea] hover:border-[#667eea]/50 transition-all duration-200"
                        ...
                        <Link
                          ...
                          className="p-2 bg-gradient-to-r from-gray-100 to-white border border-gray-200 text-gray-500 rounded-lg hover:text-blue-600 transition-all duration-200"
          ...
          {/* Pagination */}
          {totalPages > 1 && (
            <div className="flex flex-col md:flex-row items-center justify-between p-4 border-t border-gray-100 bg-gradient-to-r from-gray-50 to-white gap-4">
              <button
                onClick={handlePrevPage}
                disabled={currentPage === 1}
