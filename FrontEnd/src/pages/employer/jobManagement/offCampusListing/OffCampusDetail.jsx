...
// ==================== Main Component ====================
const OffCampusDetails = ({ job, onClose }) => {
  const jobId = job._id;
  ...
  const getApplicants = async (targetId, targetType) => {
    setLoading(true);
    setError(null);
    try {
      const response = await getApplicationsForJob(targetId, targetType, "Applied");
  ...
  // Debug logging
  console.log('🚀 Starting shortlist process...');
  console.log('Application ID:', applicationId);
  console.log('Job object:', job);
  console.log('Job Roles:', job?.jobRoles);
  console.log('Type of jobRoles:', typeof job?.jobRoles);
  
  try {
    // Get the first job role if it's an array, otherwise use the string
    let jobRoleToSend;
    
    if (Array.isArray(job?.jobRoles)) {
      if (job.jobRoles.length > 0) {
        jobRoleToSend = job.jobRoles[0];
        console.log('📊 Using first job role from array:', jobRoleToSend);
      } else {
        jobRoleToSend = 'General Role';
        console.log('⚠️ Job roles array is empty, using default');
      }
    } else if (typeof job?.jobRoles === 'string') {
      jobRoleToSend = job.jobRoles;
      console.log('📊 Using string job role:', jobRoleToSend);
    } else {
      jobRoleToSend = 'Default Role';
      console.log('⚠️ No valid job roles found, using default');
    }
    
    console.log('📤 Sending to API - Application ID:', applicationId, 'Job Role:', jobRoleToSend);
    
    // Call the API
    const response = await shortlistCandidate(applicationId, jobRoleToSend);
    ...
    if (response?.data?.success === true) {
      ...
      console.error('❌ Shortlist failed with message:', errorMsg);
      toast.error(errorMsg);
    }
  } catch (error) {
    console.error('❌ Error in shortlistApplicant function:', error);
  ...
  const handleViewResume = async (applicant) => {
    ...
    // Try multiple resume fields
    const resumeUrl = applicantData.resume || 
                      applicantData.resumeUrl || 
                      applicantData.cv ||
                      (() => {
                        // Try to extract from Cloudinary URL in stringified data
  ...
  const handleDownloadResume = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `${selectedResume.name || 'resume'}.pdf`;
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                      <button
                        ...
                        disabled={!applicantData.resume && !applicantData.resumeUrl && !applicantData.cv}
  ...
  className={`flex items-center justify-center flex-1 py-2.5 font-medium rounded-lg transition-colors duration-200 ${
    currentStatus === 'Shortlisted'
      ...
      : 'bg-white border border-gray-300 text-yellow-600 hover:bg-yellow-50 hover:border-yellow-300'
  } disabled:opacity-70`}
>
  <Star size={16} className={`mr-2 ${isSubmitting ? 'animate-pulse' : ''}`} />
...
export default OffCampusDetails;