...
const DefineHiringPreferences = ({
  ...
  const validateForm = () => {
    ...
    if (!formData.hiringLocations?.length)
      newErrors.hiringLocations = "Hiring locations are required";
  ...
  const removeJobRole = (roleToRemove) => {
    const newJobRoles =
      formData.jobRoles?.filter((role) => role !== roleToRemove) || [];
    updateFormData({ jobRoles: newJobRoles });
  };

  const removeLocation = (locationToRemove) => {
    const newLocations =
      formData.hiringLocations?.filter(
  ...
  // --- ADDED useEffect for City Data ---
  useEffect(() => {
    // 'IN' is the ISO code for India
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      // Ensure search input click doesn't close the location dropdown
      if (
    ...
    // Use mousedown listener for better handling of interactive elements
  ...
  return (
    ...
    >
      ...
      <div className="space-y-6">
        ...
        <div ref={jobRolesRef}>
          ...
          <div className="flex flex-wrap gap-2 mb-2">
            {formData.jobRoles?.map((role, index) => {
              const roleLabel =
                jobRoleOptions.find((opt) => opt.value === role)?.label || role;
              return (
                <span
                  key={index}
                ...
                {jobRoleOptions.map((option) => (
                  <div
                    ...
                    className={`px-4 py-2 hover:bg-gray-100 cursor-pointer ${
                      formData.jobRoles?.includes(option.value)
        ...
        {/* Locations Dropdown - Multi Select with Search (using Indian Cities) */}
        <div ref={locationsRef}>
          ...
          <div className="flex flex-wrap gap-2 mb-2">
            {formData.hiringLocations?.map((location, index) => {
              const locationLabel =
                filteredLocationOptions.find((opt) => opt.value === location)
                  ?.label || location;
              return (
                <span
                  key={index}
              ...
              <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-auto">
                ...
                <div className="p-2 border-b">
                  <input
                    ...
                    // Stop propagation to prevent document handler from closing the dropdown on click/typing
                ...
                {filteredLocationOptions.length > 0 ? (
                  filteredLocationOptions.map((option) => (
                    <div
                      ...
                      className={`px-4 py-2 hover:bg-gray-100 cursor-pointer ${
                        formData.hiringLocations?.includes(option.value)
                          ? "bg-gray-100 font-medium"
                          : ""
                      }`}
                    >
