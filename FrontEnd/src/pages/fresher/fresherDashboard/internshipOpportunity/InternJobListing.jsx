...
import InternshipDetailModal from '@/components/student/studentDashboard/intershipOpportunity/InternshipDetailModal';

function FInternJobListings({ compact = false, onJobSelect, selectedJobId, hideSelected = false, onJobsLoaded }) {
  ...
  const fetchInternships = async () => {
    try {
      ...
      console.log('Fresher Internships response:', response);
      
      // Access the data property from the response
      const fetchedJobs = response?.data?.data || [];
      
      setAllJobs(fetchedJobs);
      setInternships(fetchedJobs);
  ...
  const extractFilterOptions = (jobsData) => {
    ...
    jobsData.forEach(job => {
      ...
      // DURATION
      if (job.duration) {
        durations.add(job.duration.trim());
      }
    });
    ...
    setFilterOptions(prev => ({
      ...
      employmentType: employmentTypes.size > 0
        ...
        : prev.employmentType,
      duration: durations.size > 0
        ? Array.from(durations).map(label => ({ label }))
        : prev.duration,
    }));
  };
  ...
  useEffect(() => {
    ...
    if (filters.duration) {
      result = result.filter(job => 
        job.duration && job.duration.toLowerCase().includes(filters.duration.toLowerCase())
      );
    }

    // Apply STIPEND filter
    if (filters.stipend) {
      result = result.filter(job => job.stipend && job.stipend > 0);
    }
  ...
  const handleJobSelect = (job) => {
    console.log('Opening internship details for fresher:', job?.companyPosted?.companyDetails?.companyName);
    setSelectedJob(job);
    setIsModalOpen(true);
...
if (compact) {
        ...
        <div className="text-center py-8">
          ...
          <p className="text-gray-500 text-sm">
            {hideSelected && selectedJobId 
              ? "No other internships to display" 
  ...
  const clearFilterSection = (filterType) => {
    setFilters(prev => ({
      ...prev,
      [filterType]: filterType === 'stipend' ? false : []
    }));
  };
  ...
  const toggleStipendFilter = () => {
    setFilters(prev => ({
      ...prev,
      stipend: !prev.stipend
    }));
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#667eea]/10 via-[#f093fb]/5 to-[#764ba2]/10">
      ...
      <div className="relative z-10 container mx-auto px-4 py-8 pt-20">
            ...
            <div className="relative flex flex-col lg:flex-row lg:items-center justify-between py-6 px-6 gap-4">
              <div>
                ...
                <p className="text-gray-600 mt-2">
                  Explore internships tailored to your academic background and skills
                </p>
              </div>
              <div className="flex items-center space-x-2">
                <span className="text-sm text-gray-600 bg-gradient-to-r from-[#a5b4fc]/20 to-[#c4b5fd]/20 px-3 py-1 rounded-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white/90 backdrop-blur-sm border border-white/50 rounded-xl shadow-lg shadow-emerald-100/50 p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600">Paid Internships</p>
                <p className="text-2xl font-bold text-[#10b981]">
                  {allJobs.filter(job => job.stipend && job.stipend > 0).length}
                </p>
              </div>
              ...
              <div className="flex items-center flex-wrap gap-2">
                ...
                {Array.isArray(filters.location) &&
                  filters.location.map(loc => (
                    <span
                      key={loc}
                      className="inline-flex items-center bg-gradient-to-r from-[#bae6fd]/20 to-[#0ea5e9]/10 text-[#0ea5e9] px-3 py-1.5 rounded-lg text-sm backdrop-blur-sm"
                    >
                      <MapPin className="h-3 w-3 mr-1" />
                      {loc}
                      <button
                        onClick={() => removeFilter('location', loc)}
                        className="ml-2 text-[#0ea5e9] hover:text-[#0369a1]"
                      >
                        <X className="h-3 w-3" />
                      </button>
                    </span>
                ...
                {filters.stipend && (
                  <span className="inline-flex items-center bg-gradient-to-r from-[#a7f3d0]/20 to-[#10b981]/10 text-[#10b981] px-3 py-1.5 rounded-lg text-sm backdrop-blur-sm">
                    Paid Internship
                    <button 
                      onClick={() => removeFilter('stipend', true)}
                      className="ml-2 text-[#10b981] hover:text-[#059669]"
                    >
                      <X className="h-3 w-3" />
                    </button>
            ...
            <div className="mt-4 bg-white/90 backdrop-blur-sm border border-white/50 rounded-2xl shadow-lg shadow-purple-50/50 relative p-6 z-50">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative">
                ...
                <div className="relative">
                  ...
                  <button
                    onClick={() => toggleSubDropdown('duration')}
                    className="flex items-center justify-between w-full p-3 bg-gradient-to-r from-white/50 to-white/30 border border-white/50 rounded-xl hover:border-[#ddd6fe]/50 transition-all duration-200 mb-2 backdrop-blur-sm"
                  >
                    <span className="text-sm text-gray-700">Select Duration</span>
                    <ChevronDown className={`h-4 w-4 text-gray-500 transition-transform duration-200 ${openSubDropdowns.duration ? 'transform rotate-180' : ''}`} />
                  </button>
                  
                  {openSubDropdowns.duration && (
                    <div className="mt-2 p-3 bg-white/50 backdrop-blur-sm rounded-lg border border-white/50">
                      <div className="space-y-2">
                        {filterOptions.duration.map((option, index) => (
                          <div key={option.label} className="flex items-center p-2 hover:bg-white/30 rounded transition-all duration-200">
                            <input
                              type="radio"
                              id={`duration-${option.label}`}
                              name="duration"
                ...
                <div className="relative">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center">
                      ...
                      {Array.isArray(filters.location) && filters.location.length > 0 && (
                        <span className="ml-2 px-2 py-0.5 bg-[#0ea5e9] text-white text-xs rounded-full">
                  ...
                  <button
                    ...
                    className="flex items-center justify-between w-full p-3 bg-gradient-to-r from-white/50 to-white/30 border border-white/50 rounded-xl hover:border-[#bae6fd]/50 transition-all duration-200 mb-2 backdrop-blur-sm"
                  >
                  ...
                  {openSubDropdowns.location && (
                      <div className="absolute z-[100] w-full mt-2 p-3 bg-white rounded-lg border border-gray-200">
                        <CreatableSelect
                          isMulti
                          options={cityOptions}
                ...
                <div className="relative">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center">
                      <DollarSign className="h-4 w-4 text-[#10b981] mr-2" />
                    ...
                    {filters.stipend && (
                      <button
                        onClick={() => clearFilterSection('stipend')}
                        className="text-xs text-[#10b981] hover:text-[#059669] font-medium"
                      >
                        Clear
                      </button>
                  ...
                  <div className="flex items-center p-3 bg-gradient-to-r from-white/50 to-white/30 border border-white/50 rounded-xl backdrop-blur-sm">
                    <div className="flex items-center">
                      <input
                        type="checkbox"
                        id="stipend"
                        checked={filters.stipend}
                        onChange={() => toggleStipendFilter()}
                        className="h-4 w-4 text-[#10b981] focus:ring-[#a7f3d0]/50 border-gray-300 rounded"
                      />
                      <label 
                        htmlFor="stipend"
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <h3 className="text-lg font-semibold text-gray-900">
                  Showing {filteredJobs.length} of {allJobs.length} Internships
                </h3>
              ...
              <div className="text-sm text-gray-600">
                ...
                {sortBy === 'newest' ? 'Newest First' : 
                 ...
                 sortBy === 'stipend' ? 'Highest Stipend' : 
                 'Nearest Deadline'}
              </div>
            </div>
          </div>
          ...
          {filteredJobs.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center h-full text-center py-12">
              ...
              <p className="text-gray-600 mb-8 max-w-md mx-auto text-lg">
                No internships match your current filter criteria. Try adjusting your filters or search criteria to find more options.
              </p>
      ...
      {/* Internship Detail Modal for normal view */}
