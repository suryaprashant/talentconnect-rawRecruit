...
const getApplicantLogo = (applicant) => {
  ...
  // Try multiple possible paths for applicant profile image
  const possiblePaths = [
    applicant.profileImageUrl,      // Most common
    applicant.profileImage,         // Alternative field name
    applicant.photo,                // Another possible field
    applicant.image,                // Simple field name
    applicant.profilePicture,       // Another variation
    applicant.profilePic,           // Short form
    applicant.avatar,               // Common in user profiles
    applicant.profileImageUrl       // Capitalized version
  ];
  ...
  return validLogo || 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png';
};
    ...
    try {
      ...
      if (response.data) {
        ...
        const conversationUser = {
          ...
          profileImage: applicantLogo, // Use the extracted logo
          userType: 'candidate',
            ...
            <div className="flex justify-between items-start mb-6">
              <div className="flex items-center">
                ...
                {applicantLogo ? (
                  ...
                  <div className="w-20 h-20 rounded-full bg-gradient-to-br from-gray-200 to-gray-300 flex items-center justify-center text-gray-600 text-2xl font-bold mr-4">
                    {getApplicantInitials(applicant.name)}
                  </div>
                )}
                {/* Fallback for image error */}
                <div className="w-20 h-20 rounded-full bg-gradient-to-br from-gray-200 to-gray-300 hidden items-center justify-center text-gray-600 text-2xl font-bold mr-4">
                  {getApplicantInitials(applicant.name)}
                </div>
                        ...
                        >
                          ...
                          {applicantLogo ? (
                            <img 
                              ...
                              onError={(e) => {
                                console.error('Applicant logo failed to load in list:', applicantLogo);
                                e.target.style.display = 'none';
                                const fallback = e.target.nextElementSibling;
                                if (fallback) {
                                  fallback.style.display = 'flex';
                            ...
                            <div className="w-16 h-16 rounded-full bg-gradient-to-br from-gray-200 to-gray-300 flex items-center justify-center text-gray-600 text-lg mr-4 hover:bg-gray-300 transition-colors border border-gray-200">
                              {getApplicantInitials(applicant.name)}
                            </div>
                          )}
                          {/* Fallback for image error */}
                          <div className="w-16 h-16 rounded-full bg-gradient-to-br from-gray-200 to-gray-300 hidden items-center justify-center text-gray-600 text-lg mr-4 hover:bg-gray-300 transition-colors border border-gray-200">
                            {getApplicantInitials(applicant.name)}
                          </div>
                        </div>
