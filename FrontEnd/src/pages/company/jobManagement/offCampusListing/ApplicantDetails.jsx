...
const ApplicantDetails = ({ job, isVisited, onClose, onAccept, onShortlist, onReject }) => {
  ...
  const filteredApplicants = applications.filter(applicant => {
    const matchesSearch = searchQuery === '' || 
      applicant?.applicant?.name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      applicant?.applicant?.email?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      applicant?.applicant?.skills?.some(skill => 
        skill.toLowerCase().includes(searchQuery.toLowerCase())
      );
    
    const matchesStatus = statusFilter === 'all' || 
      (statusFilter === 'shortlisted' && applicant.status === 'Shortlisted') ||
      (statusFilter === 'accepted' && applicant.status === 'Accepted') ||
      (statusFilter === 'rejected' && applicant.status === 'Rejected') ||
      (statusFilter === 'applied' && (!applicant.status || applicant.status === 'Applied'));
                ...
                <div key={applicant._id} className="bg-gradient-to-r from-gray-50 to-white border border-gray-100 rounded-xl p-6 hover:shadow-lg transition-all duration-200">
                  ...
                  <div className="flex flex-wrap gap-3 pt-6 border-t border-gray-200">
                    ...
                    <button
                      ...
                      disabled={isSubmitting || applicant?.status === 'Shortlisted'}
                      className={`inline-flex items-center gap-2 px-4 py-2.5 rounded-xl transition-all duration-200 disabled:opacity-50 ${
                        applicant?.status === 'Shortlisted'
                          ? 'bg-gradient-to-r from-yellow-100 to-yellow-50 border border-yellow-200 text-yellow-700 cursor-not-allowed'
                          : 'bg-gradient-to-r from-yellow-100 to-yellow-50 border border-yellow-200 text-yellow-700 hover:bg-yellow-50 hover:border-yellow-300'
                      }`}
                    >
                      <Star size={16} />
                      Shortlist Candidate
