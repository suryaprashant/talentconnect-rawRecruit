...
  
  // NEW: State for alternate date modal and date selection
  const [showAlternateDateModal, setShowAlternateDateModal] = useState(false);
  const [selectedStartDate, setSelectedStartDate] = useState('');
  const [selectedEndDate, setSelectedEndDate] = useState('');
  const [dateError, setDateError] = useState('');
  // Retaining original dates state to display them as the college's proposal
...
  // Initialize original dates state only (not selected dates)
  useEffect(() => {
...
    try {
      // Assuming ApplyForOncampusOppurtunity handles Pool-campus via jobType
...
  // NEW: Functions for alternate date functionality
  const handleAlternateDateClick = () => {
    // **************************************************
    // FIX: Reset selected dates to empty strings here
...
    setIsSubmitting(true);
    try {
      // API call to suggest alternate dates would go here
...
      
      toast.success('Alternate dates submitted successfully!');
      setShowAlternateDateModal(false);
      
    } catch (error) {
...
  // NEW: Modal component for alternate date selection
...
            </div>
            {/* NOTE: Removed duplicate 'Looking For' card */}
          </div>
...
                className="flex items-center border border-gray-300 rounded px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 transition-colors"
              >
