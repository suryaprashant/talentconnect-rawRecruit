...
export default function CreateJob() {
  ...
  const allLocations = ["India", "Canada", "London", "New York, NY", "San Francisco, CA", "Austin, TX", "Chicago, IL", "Remote"];
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (studentStreamsDropdownRef.current && !studentStreamsDropdownRef.current.contains(event.target)) {
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-3xl mx-auto p-4">
        <div className="bg-white p-6 rounded-lg shadow-sm mb-4">
          ...
          {/* Location Multi-Select - New Design */}
              ...
              <div className="absolute z-20 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-auto">
                {allLocations.map(location => (
                  <div
                    ...
                    className={`px-4 py-2 hover:bg-gray-100 cursor-pointer ${formData.location.includes(location) ? "bg-gray-100 font-medium" : ""}`}
                  >
                    {location}
                    {formData.location.includes(location) && <span className="float-right text-gray-500">✓</span>}
                  </div>
                ))}
              </div>
            )}
