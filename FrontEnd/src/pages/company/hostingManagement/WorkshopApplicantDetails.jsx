...
import { getWorkshopRegistrations, confirmWorkshopRegistration, rejectWorkshopRegistration } from '@/lib/Company_AxiosInstance';
import toast from 'react-hot-toast';

const WorkshopApplicantDetails = ({ workshop, onClose }) => {
  ...
  const fetchRegistrations = async () => {
    setIsLoading(true);
    try {
      const response = await getWorkshopRegistrations(workshop._id);
  ...
  const handleConfirmRegistration = async (registrationId) => {
    try {
      const response = await confirmWorkshopRegistration(registrationId);
      if (response?.data?.success) {
  ...
  const handleRejectRegistration = async (registrationId, reason = '') => {
    try {
      const response = await rejectWorkshopRegistration(registrationId, { reason });
      if (response?.data?.success) {
  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      <div className="bg-green-600 text-white p-6">
        ...
        <p className="text-green-100 mt-2">Manage registrations for this workshop</p>
      </div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-400 mt-2">
                {filter === 'all' ? 'No one has registered for this workshop yet.' : `No ${filter} registrations found.`}
              </p>
            </div>
          ) : (
...
export default WorkshopApplicantDetails;
