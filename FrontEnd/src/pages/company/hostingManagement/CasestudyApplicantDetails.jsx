...
import { getCasestudyRegistrations, confirmCasestudyRegistration, rejectCasestudyRegistration } from '@/lib/Company_AxiosInstance';
import toast from 'react-hot-toast';

const CasestudyApplicantDetails = ({ casestudy, onClose }) => {
  ...
  const fetchRegistrations = async () => {
    setIsLoading(true);
    try {
      const response = await getCasestudyRegistrations(casestudy._id);
  ...
  const handleConfirmRegistration = async (registrationId) => {
    try {
      const response = await confirmCasestudyRegistration(registrationId);
      if (response?.data?.success) {
  ...
  const handleRejectRegistration = async (registrationId, reason = '') => {
    try {
      const response = await rejectCasestudyRegistration(registrationId, { reason });
      if (response?.data?.success) {
  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      <div className="bg-[#143694] text-white p-6">
        ...
        <p className="text-purple-100 mt-2">Manage registrations for this case study</p>
      </div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-400 mt-2">
                {filter === 'all' ? 'No one has registered for this case study yet.' : `No ${filter} registrations found.`}
              </p>
            </div>
          ) : (
...
export default CasestudyApplicantDetails;
