...
const ApplicantCard = ({ applicationData, jobRole, onStatusChange }) => {
    ...
    const getButtonText = (buttonType, currentStatus, isProcessing) => {
        if (isProcessing) {
            switch (buttonType) {
                case 'shortlist':
                    return currentStatus === 'Shortlisted' ? 'Shortlisted' : 'Shortlisting...';
                case 'reject':
                    return currentStatus === 'Rejected' ? 'Rejected' : 'Rejecting...';
                case 'accept':
                    return currentStatus === 'Accepted' ? 'Accepted' : 'Accepting...';
                default:
                    return '';
            }
        }
        ...
        switch (buttonType) {
            case 'shortlist':
                return currentStatus === 'Shortlisted' ? 'Shortlisted' : 'Shortlist';
            case 'reject':
                return currentStatus === 'Rejected' ? 'Rejected' : 'Reject';
            case 'accept':
                return currentStatus === 'Accepted' ? 'Accepted' : 'Accept';
            default:
                return '';
        }
    };
