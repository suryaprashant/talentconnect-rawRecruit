...
function Home() {
  ...
  const recentUpdates = useMemo(() => {
    const onCampusUpdates = dashboardData.onCampus?.updates || []
    const poolCampusUpdates = dashboardData.poolCampus?.updates || []
    
    const items = [...onCampusUpdates, ...poolCampusUpdates]
    items.sort((a, b) => new Date(b?.createdAt || 0) - new Date(a?.createdAt || 0))
    return items.slice(0, 6)
  }, [dashboardData.onCampus?.updates, dashboardData.poolCampus?.updates])
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (metricsResponse.data?.success) {
        ...
        setDashboardData({
          loading: false,
          onCampus: metricsData.onCampus || {
            total: 0,
            byStatus: { Applied: 0, Shortlisted: 0, Accepted: 0, Rejected: 0 },
            updates: [],
            items: [],
          },
          poolCampus: metricsData.poolCampus || {
            total: 0,
            byStatus: { Applied: 0, Shortlisted: 0, Accepted: 0, Rejected: 0 },
            updates: [],
            items: [],
  ...
  const onCampusTotal = dashboardData.onCampus?.total || 0
  const poolCampusTotal = dashboardData.poolCampus?.total || 0
  const onCampusUpdatesCount = dashboardData.onCampus?.updates?.length || 0
  const poolCampusUpdatesCount = dashboardData.poolCampus?.updates?.length || 0
    ...
    <div>
      ...
      <div className="grid grid-cols-1 gap-6 mt-4 md:grid-cols-2">
        ...
        >
          <h2 className="mb-2 text-lg font-medium text-gray-900">On-Campus Applications</h2>
          <p className="text-3xl font-bold text-blue-600">{onCampusTotal}</p>
          <p className="mt-2 text-xs text-gray-500">Updates: {onCampusUpdatesCount}</p>
        </div>
        ...
        >
          <h2 className="mb-2 text-lg font-medium text-gray-900">Pool-campus Applications</h2>
          <p className="text-3xl font-bold text-indigo-600">{poolCampusTotal}</p>
          <p className="mt-2 text-xs text-gray-500">Updates: {poolCampusUpdatesCount}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 mt-6 md:grid-cols-2">
        <div className="p-6 bg-white border border-gray-200 rounded-lg shadow-sm">
          <h3 className="text-sm font-semibold text-gray-900">On-campus Status</h3>
          <ul className="mt-3 grid grid-cols-2 gap-3 text-sm">
            <li className="flex items-center justify-between"><span className="text-gray-500">Applied</span><span className="font-medium">{dashboardData.onCampus?.byStatus?.Applied || 0}</span></li>
            <li className="flex items-center justify-between"><span className="text-gray-500">Shortlisted</span><span className="font-medium text-yellow-600">{dashboardData.onCampus?.byStatus?.Shortlisted || 0}</span></li>
            <li className="flex items-center justify-between"><span className="text-gray-500">Accepted</span><span className="font-medium text-green-600">{dashboardData.onCampus?.byStatus?.Accepted || 0}</span></li>
            <li className="flex items-center justify-between"><span className="text-gray-500">Rejected</span><span className="font-medium text-red-600">{dashboardData.onCampus?.byStatus?.Rejected || 0}</span></li>
          </ul>
        </div>
        ...
        <div className="p-6 bg-white border border-gray-200 rounded-lg shadow-sm">
          <h3 className="text-sm font-semibold text-gray-900">Pool-campus Status</h3>
          <ul className="mt-3 grid grid-cols-2 gap-3 text-sm">
            <li className="flex items-center justify-between"><span className="text-gray-500">Applied</span><span className="font-medium">{dashboardData.poolCampus?.byStatus?.Applied || 0}</span></li>
            <li className="flex items-center justify-between"><span className="text-gray-500">Shortlisted</span><span className="font-medium text-yellow-600">{dashboardData.poolCampus?.byStatus?.Shortlisted || 0}</span></li>
            <li className="flex items-center justify-between"><span className="text-gray-500">Accepted</span><span className="font-medium text-green-600">{dashboardData.poolCampus?.byStatus?.Accepted || 0}</span></li>
            <li className="flex items-center justify-between"><span className="text-gray-500">Rejected</span><span className="font-medium text-red-600">{dashboardData.poolCampus?.byStatus?.Rejected || 0}</span></li>
          </ul>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 mt-6 md:grid-cols-2">
        <div className="p-6 bg-white border border-gray-200 rounded-lg shadow-sm">
          <h3 className="text-sm font-semibold text-gray-900">Service Requests</h3>
          <ul className="mt-3 grid grid-cols-2 gap-3 text-sm">
            <li className="flex items-center justify-between"><span className="text-gray-500">Total</span><span className="font-medium">{dashboardData.serviceRequests?.total || 0}</span></li>
            <li className="flex items-center justify-between"><span className="text-gray-500">Pending</span><span className="font-medium text-yellow-600">{dashboardData.serviceRequests?.pending || 0}</span></li>
            <li className="flex items-center justify-between"><span className="text-gray-500">Approved</span><span className="font-medium text-green-600">{dashboardData.serviceRequests?.approved || 0}</span></li>
            <li className="flex items-center justify-between"><span className="text-gray-500">Rejected</span><span className="font-medium text-red-600">{dashboardData.serviceRequests?.rejected || 0}</span></li>
          </ul>
        </div>
      </div>
