...
import { MapPin, Clock, Calendar, Building, Award, ArrowRight, CheckCircle, FileText, Users, Eye, TrendingUp, Search, Briefcase, GraduationCap } from 'lucide-react';
...
export default function PoolCampusApplicationStatus() {
    ...
    const fetchApplication = async () => {
        try {
            ...
            const normalized = rawData.map((item) => {
                ...
                let locationDisplay = "-";
                if (safeJobDetails[0]?.location && Array.isArray(safeJobDetails[0].location)) {
                    locationDisplay = safeJobDetails[0].location.slice(0, 2).join(", ");
                } else if (safeJobDetails[0]?.workLocations) {
                    locationDisplay = safeJobDetails[0].workLocations;
                }

                // Format job title/roles
                let jobTitleDisplay = "N/A";
                if (safeJobDetails[0]?.jobRoles && Array.isArray(safeJobDetails[0].jobRoles)) {
                    jobTitleDisplay = safeJobDetails[0].jobRoles.join(", ");
                } else if (safeJobDetails[0]?.jobTitle) {
                    jobTitleDisplay = safeJobDetails[0].jobTitle;
                }
                ...
                return {
                    ...
                    company: safeCompanyDetails[0].companyDetails.companyName,
                    jobTitle: jobTitleDisplay,
                    employmentType: safeJobDetails[0]?.employmentType || "-",
                    location: locationDisplay,
                    experience: item?.experience ?? safeJobDetails[0]?.yearsOfExperience ?? "-",
                    description: safeJobDetails[0]?.jobDescription || safeJobDetails[0]?.description || "No description available",
            ...
            const sortedApplications = [...normalized].sort((a, b) => {
                switch (sortBy) {
                    case "oldest":
            ...
            setPoolcampusJobs(sortedApplications);
            if (sortedApplications.length > 0) {
                setSelectedJob(sortedApplications[0]);
            }
        } catch (error) {
    ...
    // Calculate stats
    const totalApplications = poolcampusJobs.length;
    const acceptedCount = poolcampusJobs.filter(job => job.status === 'Accepted').length;
    const inProgressCount = poolcampusJobs.filter(job => ['Applied', 'Shortlisted'].includes(job.status)).length;
                        ...
                        <div className="flex flex-col sm:flex-row gap-3">
                            <div className="relative">
                                <input
                                    ...
                                    className="pl-10 pr-4 py-2.5 bg-white/90 backdrop-blur-sm border border-white/50 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#93c5fd] focus:border-transparent w-full"
                                    value={searchTerm}
                                    onChange={(e) => setSearchTerm(e.target.value)}
                                />
                                <Search className="absolute left-3 top-3 h-3.5 w-3.5 text-[#3b82f6]" />
                            </div>
                            <div className="relative">
                                <select
                                    className="w-full pl-4 pr-10 py-2.5 bg-white/90 backdrop-blur-sm border border-white/50 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#93c5fd] focus:border-transparent appearance-none"
                                    value={sortBy}
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        ...
                                                        <div className="space-y-1">
                                                            <div className="flex items-center text-xs text-gray-500">
                                                                <Clock className="w-3 h-3 mr-1.5 text-[#3b82f6] flex-shrink-0" />
