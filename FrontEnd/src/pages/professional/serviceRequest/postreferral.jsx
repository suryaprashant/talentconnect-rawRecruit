...
function PostReferralJobPage() {
    ...
    // --- Merged Submission and Cancellation Handlers ---
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await postReferralJob(payload); // Using the original API function
            console.log('API Response:', response);
            toast.success('Referral job posted successfully!');
            setFormData(initialState); // Reset form
            navigate('/professional/service-request'); // Navigate on success
        } catch (err) {
    ...
    const handleCancel = () => {
        setFormData(initialState); // Reset form state
        navigate('/professional/service-request'); // Navigate back
    };
                        ...
                        <div className="relative p-2 border border-gray-300 rounded-md" onClick={() => setDropdownOpen(prev => ({ ...prev, tags: true }))}>
                            ...
                            <div className="w-full cursor-pointer">
                                <div className="flex items-center justify-between text-gray-500">
                                    <span>{formData.tags.length > 0 ? '' : 'Select tags'}</span>
