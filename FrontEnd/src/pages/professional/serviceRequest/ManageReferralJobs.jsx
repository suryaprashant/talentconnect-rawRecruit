...
// This component shows all applicants for a *selected* referral job

const ReferralApplicantDetails = ({ job, onClose }) => {
  const jobId = job._id;
  const jobType = job.jobType; // This will be "Referral"
  ...
  const acceptApplicant = async (applicantionId) => {
    try {
      ...
      if (response?.data?.success === true) {
        ...
        getApplicants(jobId, jobType); // Refresh list
      } else {
  ...
  const handleMessageClick = async (applicant) => {
    ...
    try {
      const response = await conversationWithCollege(userId); // Assuming this can chat with any user
      if (response.data) {
  ...
  return (
    <>
      <button onClick={onClose} className="mb-4 text-blue-600 hover:underline">
        &larr; Back to Job List
      </button>
      <div className="bg-white min-h-screen flex">
        {/* Filters Sidebar (Static as per your example) */}
        ...
        <div className="flex-1 p-8">
          <h1 className='text-2xl font-bold mb-6'>{job?.jobRoles[0]}</h1>
          {isSubmitting && <p>Loading applicants...</p>}
          {!isSubmitting && applications.length === 0 && <p>No applicants found for this job.</p>}
                ...
                <div className="p-6">
                  <div className="flex justify-between items-start mb-6">
                    <div className="flex items-center">
                      <img 
                        src={applicant?.applicant.profileImageUrl || 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png'} 
                        alt={applicant?.applicant.name}
                        className="w-20 h-20 bg-gray-200 rounded-full mr-6 object-cover"
                      />
                      <div>
                  ...
                  <div className="flex space-x-4 mb-8">
                    ...
                    {applicant?.applicant.linkedIn && <a href={applicant.applicant.linkedIn} target="_blank" rel="noreferrer" className="px-4 py-2 border rounded-md text-sm">LinkedIn</a>}
                    {applicant?.applicant.github && <a href={applicant.applicant.github} target="_blank" rel="noreferrer" className="px-4 py-2 border rounded-md text-sm">GitHub</a>}
                    {applicant?.applicant.portfolio && <a href={applicant.applicant.portfolio} target="_blank" rel="noreferrer" className="px-4 py-2 border rounded-md text-sm">Portfolio</a>}
                  </div>
...
// This is the main dashboard page that lists all referral jobs in a table

export default function ReferralJobManagement() {
  const [jobs, setJobs] = useState([]);
  ...
  const fetchJobs = async () => {
    setLoading(true);
    try {
      // *** KEY CHANGE: Fetching "Referral" jobs instead of "Job-listing" ***
      const response = await getPostedJobs("Referral", "Applied");
      setJobs(response?.data || []);
    } catch (error) {
      console.error("Error fetching jobs:", error);
      setJobs([]); // Ensure jobs is an array on error
    }
    setLoading(false);
  };
  ...
  const filteredJobs = jobs?.filter(job => {
    const matchesSearch = job.jobTitle.toLowerCase().includes(searchQuery.toLowerCase()) ||
      ...
      // job.venue.toLowerCase().includes(searchQuery.toLowerCase()); // 'venue' might not exist on all jobs
    ...
    return matchesSearch;
  }) || []; // Ensure filteredJobs is always an array
  ...
  if (showApplicantDetails && selectedJob) {
    return (
      <div className="max-w-7xl mx-auto p-4">
        <ReferralApplicantDetails
          job={selectedJob}
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-7xl mx-auto p-4 bg-white">
        <div className="flex justify-between items-center mt-10 mb-4">
          <div>
            {/* *** KEY CHANGE: Title updated *** */}
            <h1 className="text-3xl font-bold">Manage Referral Job Applications</h1>
            <p className="text-gray-600 mt-2">Track Your Referral Job Listings and Streamline Candidate Applications</p>
          </div>
        </div>
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan="5" className="text-center py-4 text-gray-500">
                      No referral jobs found matching your criteria.
                    </td>
                  </tr>
                ) : (
                  currentJobs.map(job => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">{new Date(job.endDate ? job.endDate : job?.expireAt).toUTCString().slice(0, 16)}</td>
                      <td className="px-4 py-3">{job?.applicationCount}</td>
                      <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                        <div className="flex gap-2">
                          <button onClick={() => handleView(job._id)} className="text-gray-500 hover:text-gray-700" title="View Job Applicants">
                            <Eye size={18} />
                          </button>
                          {/* *** KEY CHANGE: Link updated for "Referral-Job" *** */}
                          <Link
                            to={`/company-dashboard/Referral-Job/${job._id}?isApplied=true`}
