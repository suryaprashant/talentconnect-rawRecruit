import { Outlet } from "react-router-dom";
import PoolJobListingPage from "@/pages/college/collegeDashboard/poolCampusOpportunity/PoolJobListingPage";

const PoolCampusLayout = () => {
  return (
    <div className="flex h-[calc(100vh-64px)] overflow-hidden bg-white">
      {/* LEFT SIDE: The dynamic detail content */}
      <div className="flex-1 overflow-y-auto border-r bg-white">
        <Outlet />
      </div>

      {/* RIGHT SIDE: The persistent sidebar list */}
      <div className="w-[400px] overflow-y-auto bg-gray-50 border-l hidden lg:block">
        <div className="p-4 border-b bg-white sticky top-0 z-10">
          <h2 className="font-bold text-gray-700">All Pool Opportunities</h2>
        </div>
        {/* Forces the list to show in simplified mode */}
        <PoolJobListingPage compact={true} />
      </div>
    </div>
  );
};
export default PoolCampusLayout;