...
const InternshipStatus = () => {
  ...
  // Define internship specific status steps - Changed to match off-campus pattern
      ...
      const detailedJobs = await Promise.all(
        rawData.map(async (item) => {
          try {
            ...
            // Map status to match new status steps
            const rawStatus = item.currentStatus || item.status || "Applied";
            let mappedStatus = rawStatus;
            
            // Map old statuses to new ones
            if (rawStatus.toLowerCase() === 'under review' || rawStatus.toLowerCase() === 'review') {
              mappedStatus = 'Shortlisted';
            } else if (rawStatus.toLowerCase() === 'interview' || rawStatus.toLowerCase() === 'interviewed') {
              mappedStatus = 'Shortlisted';
            } else if (rawStatus.toLowerCase() === 'selected') {
            ...
            return {
              ...
              jobId: jobId,
              status: mappedStatus,
              rawStatus: rawStatus, // Keep original for display
              ...
              _debug: {
                ...
                statusMapping: `${rawStatus} → ${mappedStatus}`
              }
            };
  ...
  const getStatusIndex = (status) => {
    ...
    // For normal progression using new status steps
    if (lowerStatus === 'applied') return 0;
    if (lowerStatus === 'shortlisted') return 1;
    if (lowerStatus === 'accepted') return 2;
    
    // Fallback for any remaining old statuses
    if (lowerStatus === 'under review' || lowerStatus === 'review') return 1;
    if (lowerStatus === 'interview' || lowerStatus === 'interviewed') return 1;
    if (lowerStatus === 'selected') return 2;
  ...
  const getStatusColor = (status) => {
    switch(status.toLowerCase()) {
      ...
      case 'review': 
      case 'interview': 
      case 'interviewed': return 'bg-gradient-to-r from-[#fde68a]/20 to-[#fcd34d]/20 text-[#92400e] border border-[#fde68a]/30';
      case 'accepted': 
      case 'selected': return 'bg-gradient-to-r from-[#bbf7d0]/20 to-[#86efac]/20 text-[#065f46] border border-[#bbf7d0]/30';
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="mt-1.5 flex items-center text-xs text-gray-500 gap-2">
                              ...
                              <span className={`text-xs px-1.5 py-0.5 rounded ${
                                job.status === 'Accepted' || job.rawStatus === 'Selected' 
                                  ? 'bg-green-100 text-green-700 border border-green-200' :
                                job.status === 'Shortlisted' || 
                                job.rawStatus === 'Under Review' || 
                                job.rawStatus === 'Interview' ||
                                job.rawStatus === 'Interviewed' 
                                  ? 'bg-amber-100 text-amber-700 border border-amber-200' :
                                job.status === 'Rejected' 
                                  ? 'bg-red-100 text-red-700 border border-red-200' :
                                'bg-purple-100 text-purple-700 border border-purple-200'
                              }`}>
                                {job.rawStatus || job.status}
                              </span>
                            </div>
                          </div>
                        </div>
            ...
            {selectedJob ? (
              <div className="bg-white/90 backdrop-blur-sm border border-white/50 rounded-2xl shadow-lg shadow-purple-50/50 h-full flex flex-col">
                ...
                <div className="p-5 border-b border-white/60">
                  ...
                  {/* Updated Progress Bar for Internship - Same as Off-Campus */}
                  {selectedJob.status.toLowerCase() === 'rejected' ? (
                    ...
                    // Normal Status Flow - 3-step bar (Applied, Shortlisted, Accepted)
                    <div className="relative">
                      <div className="flex justify-between mb-1">
