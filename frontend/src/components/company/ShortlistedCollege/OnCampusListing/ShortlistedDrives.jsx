...
const ShortlistedDrives = ({
  ...
  const handleManageDrive = (driveId) => {
    navigate(`/shortlisted/on-campus-listings/${driveId}`);
  };
      ...
      <div className="flex justify-between items-center mt-8">
        ...
        <div className="flex gap-2">
          {pageNumbers.map(num => (
            <button
              key={num}
              className={`w-8 h-8 flex items-center justify-center rounded-md ${currentPage === num ? 'bg-black text-white' : 'border'
                }`}
