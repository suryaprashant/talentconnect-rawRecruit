...
//                   if (!localFormData.skills.includes(selected)) {
//                     setLocalFormData((prev) => ({
//                       ...prev,
...
//           </div>
          ...
          //     <input
          //       id="linkedinProfile"
          //       name="linkedin" // This should be "linkedin"
          ...
          //       {localFormData.project ? localFormData.project.name : "Upload PDF"}
          //     </span>
...
export const StepFive = ({ onNext, onBack, formData, onChange }) => {
  ...
  const handleSkillSelect = (e) => {
      ...
      if (selectedSkill && !localFormData.skills.includes(selectedSkill)) {
          setLocalFormData(prev => ({ ...prev, skills: [...prev.skills, selectedSkill] }));
      }
  };

  const removeSkill = (skillToRemove) => {
      setLocalFormData(prev => ({ ...prev, skills: prev.skills.filter(skill => skill !== skillToRemove) }));
  };
    ...
    <div className="justify-center items-stretch bg-white z-0 flex min-w-60 flex-col w-[560px] my-auto p-12 max-md:max-w-full max-md:px-5">
      ...
      <div className="flex w-full flex-col items-stretch justify-center mt-8 max-md:max-w-full">
        ...
        <form className="w-full text-base font-normal mt-8 max-md:max-w-full">
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {localFormData.skills.map((skill, index) => (
                <span key={index} className="flex items-center bg-gray-200 text-sm text-black px-3 py-1 rounded-full">
                  {skill}
                  <button type="button" onClick={() => removeSkill(skill)} className="ml-2 text-gray-600 hover:text-black">×</button>
                </span>
              ))}
            </div>
            <div className="relative mt-2">
              <select id="skills" name="skills" value="" onChange={handleSkillSelect} className="items-center appearance-none flex min-h-12 w-full gap-2 text-[#666] p-3 border border-gray-300 rounded">
          ...
          {/* Certifications */}
          <div className="w-full whitespace-nowrap mt-6 max-md:max-w-full">
            <label htmlFor="certifications" className="block text-black max-md:max-w-full">Certifications</label>
            <textarea id="certifications" name="certifications" value={localFormData.certifications} onChange={handleChange} className="w-full p-3 mt-2 border border-gray-300 rounded min-h-[80px]" placeholder="Placeholder"></textarea>
          </div>
          
          {/* Social Links, Project, Referral... */}
          {/* (No changes needed in the JSX for the rest of this file) */}
                     <div className="w-full mt-6 max-md:max-w-full">
            <label
              htmlFor="linkedinProfile"
              className="block text-black max-md:max-w-full"
          ...
          <div className="flex min-h-12 w-full gap-2.5 whitespace-nowrap mt-6 max-md:max-w-full">
            <div className="flex gap-4">
              <button type="button" onClick={onBack} className="self-stretch gap-2 text-black border rounded-md px-6 py-3 max-md:px-5 cursor-pointer">
                Back
              </button>
              <button type="button" onClick={handleNextClick} className="self-stretch bg-black border rounded-md gap-2 text-white px-6 py-3 max-md:px-5 cursor-pointer">
                Next
              </button>
            </div>
          </div>
