...
// export const FresherStepFour = ({ onNext, onBack, formData, onChange }) => {
...
//         >
//           <span>
//             {tempFormData[field].length > 0
//               ? tempFormData[field].join(", ")
...
//                 className={`p-2 hover:bg-gray-100 cursor-pointer ${tempFormData[field].includes(option) ? "bg-gray-200" : ""
//                   }`}
//               >
...
//   const handleClick = () => {
//     onChange({ ...formData, ...tempFormData });
...
//                   name="expectedSalary"
//                   type="text"
//                   value={tempFormData.expectedSalary}
//                   onChange={handleChange}
...
//                   key={option}
//                   type="button"
//                   className={`self-stretch gap-2 px-4 py-2 border rounded-md ${tempFormData.lookingFor === option ? "bg-black text-white" : ""
//                     }`}
...
//                   key={option}
//                   type="button"
//                   className={`self-stretch gap-2 px-4 py-2 border rounded-md ${tempFormData.employmentType === option
//                       ? "bg-black text-white"
//                       : ""
//                     }`}
...
//                 value={tempFormData.internshipCompany}
//                 onChange={handleChange}
...
// import { useRole } from "@/context/RoleContext/RoleContext"; // CHANGE: Import useRole to access global state

// export const FresherStepFour = ({ onNext, onBack }) => {
//   // CHANGE: Get formData and updateFormData directly from the global context
//   const { formData, updateFormData } = useRole();

//   // Local state is now only for controlling UI elements, not for form data
//   const [dropdownOpen, setDropdownOpen] = useState({
//     industry: false,
//     jobRoles: false,
//     locations: false,
...
//   // CHANGE: All handlers now update the global formData state directly
//   const handleSelect = (field, value) => {
...
//   const handleChange = (e) => {
//     const { name, value } = e.target;
//     // FIX: Map frontend input names to the correct backend schema field names
//     if (name === "expectedSalaryAmount") {
//       updateFormData({ expectedSalaryAmount: value });
//     } else if (name === "expectedSalaryCurrency") {
...
//   // --- FIX: Logic for adding, updating, and removing experiences ---
//   const handleAddExperience = () => {
//     const currentExperiences = formData.experiences || [];
//     const newExperiences = [...currentExperiences, { company: "", role: "", startDate: "", endDate: "", description: "" }];
...
//                   (formData[field] || []).includes(option.value) ? "bg-gray-200" : ""
//                 }`}
//               >
//                 {option.label}
//               </li>
...
//   return (
//     <div className="justify-center items-stretch bg-white z-0 flex min-w-60 flex-col w-[560px] my-auto p-12 max-md:max-w-full">
//       <ProgressIndicator currentStep={4} totalSteps={5} />
...
//         <form className="w-full text-base font-normal mt-8 max-md:max-w-full">
//           {/* FIX: Use correct schema field name 'industry' */}
...
//           {renderCustomDropdown("jobRoles", 
//             [ { value: "developer", label: "Developer" }, { value: "designer", label: "Designer" }, { value: "manager", label: "Manager" } ],
//             "Interested Job Roles"
//           )}

//           {renderCustomDropdown("locations", 
//             [ { value: "bangalore", label: "Bangalore" }, { value: "mumbai", label: "Mumbai" }, { value: "remote", label: "Remote" } ],
//             "Preferred Job Locations"
//           )}

//           {/* FIX: Bind to correct formData fields */}
//           <div className="w-full mt-6 max-md:max-w-full">
//             <label htmlFor="expectedSalary" className="block text-black">Expected Salary</label>
//             <div className="flex items-center gap-2 mt-2">
//               <div className="relative w-24">
//                 <select
...
//           {/* --- FIX: Dynamic Internships/Trainings section --- */}
//           <div className="w-full mt-6 max-md:max-w-full">
...
//                   <input name="company" value={exp.company} onChange={(e) => handleExperienceChange(index, e)} placeholder="Company Name" className="w-full min-h-12 p-3 border border-gray-300 rounded"/>
//                 </div>
//                 <div className="w-full mt-4">
//                   <input name="role" value={exp.role} onChange={(e) => handleExperienceChange(index, e)} placeholder="Job Role (e.g., Frontend Developer Intern)" className="w-full min-h-12 p-3 border border-gray-300 rounded"/>
//                 </div>
//                 <div className="flex w-full gap-6 mt-4">
//                   <input type="text" name="startDate" value={exp.startDate} onChange={(e) => handleExperienceChange(index, e)} placeholder="Start Date (e.g., Jan 2025)" className="flex-1 min-h-12 p-3 border border-gray-300 rounded" />
//                   <input type="text" name="endDate" value={exp.endDate} onChange={(e) => handleExperienceChange(index, e)} placeholder="End Date (e.g., Mar 2025)" className="flex-1 min-h-12 p-3 border border-gray-300 rounded" />
//                 </div>
//                 <div className="w-full mt-4">
//                     <textarea name="description" value={exp.description} onChange={(e) => handleExperienceChange(index, e)} placeholder="Describe your role and responsibilities..." className="flex min-h-24 w-full p-3 border border-gray-300 rounded"></textarea>
//                 </div>
//               </div>
//             ))}
//             <div className="flex justify-end mt-4">
...
//               <button
//                 type="button"
//                 onClick={onNext} // CHANGE: Directly call onNext, no intermediate handler needed
//                 className="self-stretch bg-black gap-2 text-white px-6 py-3 border rounded-md max-md:px-5 cursor-pointer"
//               >
//                 Next
//               </button>
