...
const DefineHiringPreferences = ({ onBack, formData,onNext, updateFormData, onSubmit }) => {
  ...
  // The onSubmit prop is passed down, but not directly used in this component's local handleSubmit,
  // as the next button just triggers onNext(). The final submit is on the T&C page.
  // However, I've kept the prop name consistent with your setup.
  // The actual form submission will occur via onNext, then TermsAndConditions, then onSubmit.

  const handleNextClick = () => { // Renamed to avoid confusion with the form's final onSubmit
    if (validateForm()) {
      onNext(); // This navigates to the next step (TermsAndConditions)
    }
  };
