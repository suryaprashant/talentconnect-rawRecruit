    ...
    <div className="container mx-auto px-4 py-8">
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="flex flex-col md:flex-row justify-between mb-6">
          <div>
            <div className="mb-2 flex items-center">
              {/* Display the job title here from the JobPostingTable schema */}
              <h1 className="text-2xl font-bold">Registration for: {jobPosting?.jobTitle || 'N/A'}</h1>
            </div>
            <h2 className="text-3xl font-bold mb-2">{collegeDetails?.collegeUniversityDetails?.collegeName || 'N/A'}</h2>
            <div className="flex items-center text-sm text-gray-600 mb-1">
            ...
            <div className="flex items-center mb-1">
              ...
              <span className="text-gray-600 text-sm">{`${formatDate(jobPosting?.startDate)} - ${formatDate(jobPosting?.endDate)}`}</span>
            </div>
          </div>
        ...
        <div className="border-t border-gray-200 pt-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            ...
            <div className="bg-white p-4 rounded border border-gray-200">
              <h4 className="font-bold text-3xl text-blue-600">{jobPosting?.numberOfOpenings || 'N/A'}</h4>
          ...
          <div className="mb-8">
            ...
            <table className="w-full border-collapse mb-6">
              ...
              <tbody>
                {/* Loop through studentStreams to generate a row for each branch */}
                {jobPosting?.studentStreams?.length > 0 ? (
                  jobPosting.studentStreams.map((branch, index) => (
                    <tr key={index}>
                      <td className="border border-gray-200 p-2">{index + 1}</td>
                      {/* You can display the number of students here if you have it per stream */}
                      <td className="border border-gray-200 p-2">{jobPosting?.numberOfStudent || 'N/A'}</td>
                      <td className="border border-gray-200 p-2">{branch}</td>
                      <td className="border border-gray-200 p-2">{jobPosting?.skills?.join(', ') || 'N/A'}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
                    <td colSpan="4" className="border border-gray-200 p-4 text-center text-gray-500">No student batch details available.</td>
                  </tr>
                )}
              </tbody>
            </table>
